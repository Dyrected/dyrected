...
- `npx dyrected generate:types` creates a standalone typed contract file when inference alone is not the right boundary
...
3. derive the client type with `InferSchema`, or generate `dyrected-types.ts` when you need a standalone contract
...
- [SDK Overview](/new-docs/managing-data/sdk-api/overview)
...
- use `generate:types` when another app, repo, or package needs a plain generated contract file
