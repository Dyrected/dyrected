...
For how the token is signed and verified, see [JWT Strategy](/new-docs/features/authentication/jwt-strategy).
...
- **A separate service that only has the token sees only identity.** If another system verifies a Dyrected token directly, it gets `sub`, `email`, and `collection` — to act on roles it must look the user up by `sub`. See [JWT Strategy](/new-docs/features/authentication/jwt-strategy#verifying-a-token-in-another-service).

## Where to go next

- [JWT Strategy](/new-docs/features/authentication/jwt-strategy) — signing, expiry, and external verification
- [Access Control](/new-docs/basics/access-control/overview) — turn the `user` object into permissions
- [Operations](/new-docs/features/authentication/operations) — the endpoints that issue these tokens
