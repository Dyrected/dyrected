...
For the exact type signature, the root configuration reference still lives on [Configuration Overview](/new-docs/basics/configuration/overview).
...
| Invite | authenticated `invite` call | `{ token, invitedByEmail? }` |
...
- `forgot-password` can receive a `resetUrl`; if it does, the reset email gets both the raw token and the full link
- invite tokens currently expire in 7 days, and reset tokens expire in 1 hour

For the endpoint-by-endpoint behavior, use [Authentication Operations](/new-docs/features/authentication/operations).
...
Keep the auth endpoint semantics on [Authentication Operations](/new-docs/features/authentication/operations). This page is for the transport setup, the built-in email triggers, and template customization.
...
For broader app-level email outside auth, plan on a separate page. This overview stays focused on the built-in auth emails and the top-level transport setup.
