...
- [Connecting your frontend](/new-docs/features/live-preview/frontend) — the overlay model behind live data.
- [Client-side integration](/new-docs/features/live-preview/client-side) — wiring `useLivePreview`, `useDyPath`, and `<Blocks>` into your pages (`postMessage`).
- [Server-side integration](/new-docs/features/live-preview/server-side) — the `token` flow for server-rendered and statically generated sites.
