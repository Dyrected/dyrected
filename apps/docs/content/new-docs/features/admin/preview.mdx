...
`postMessage` is the simpler path and needs no extra endpoint. `token` exists for frontends that render on the server and can't receive a browser message directly.

## Wiring up the frontend

Setting `previewUrl` gives you the link and opens the pane, but the *live* part — your page re-rendering as the editor types — is work your frontend does. It listens for the draft data and re-renders with it. That client-side flow, including the `useLivePreview` hook and click-to-edit field mapping, is its own feature:

- [Live Preview overview](/new-docs/features/live-preview/overview) — how the pane and your frontend talk to each other.
- [Live Preview on the frontend](/new-docs/features/live-preview/frontend) — wiring `useLivePreview` into your pages.
- [Server-side preview](/new-docs/features/live-preview/server-side) — the `token` flow for statically rendered sites.
