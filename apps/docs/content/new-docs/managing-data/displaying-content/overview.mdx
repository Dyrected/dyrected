...
An [upload](/new-docs/basics/fields/image) field, once hydrated, gives you the upload document — its `url`, `width`, `height`, and `mimeType`. `DyrectedImage` renders it as an `<img>` (Next uses `next/image`, Nuxt uses `<NuxtImg>`), filling in dimensions and alt text from the document when you don't pass them:
