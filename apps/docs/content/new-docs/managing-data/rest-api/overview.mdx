...
Most JavaScript and TypeScript apps should use the [SDK](/new-docs/managing-data/sdk-api/overview) instead of calling REST directly. Reach for REST when you need a language the SDK does not cover, or when you are integrating from another system.
...
Operators include `equals`, `not_equals`, `contains`, `starts_with`, `in`, `not_in`, `gt`, `gte`, `lt`, `lte`, and `exists`. List responses use a shared pagination envelope with `docs`, `total`, `limit`, `page`, `totalPages`, `hasNextPage`, and `hasPrevPage`.
...
Regenerate whenever the content contract changes. Do not treat an old downloaded specification as permanently compatible. For JavaScript and TypeScript, the [SDK](/new-docs/managing-data/sdk-api/overview) already gives you a typed client with no code generation step.
...
The representative document currently contains **51 operations**. Use the runtime document for client generation because its schemas reflect your own collections and globals.
