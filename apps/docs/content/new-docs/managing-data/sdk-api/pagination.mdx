...
Two options steer pagination. `limit` sets how many documents a page holds (default `10`), and `page` picks which page to return (default `1`):
...
Dyrected always paginates list reads, so this loop — rather than a single unbounded request — is the way to pull an entire collection. A larger `limit` means fewer round-trips; keep it to a sane page size rather than a number that would return tens of thousands of documents at once.
...
If you already have the document's ID, skip pagination entirely and use [`findOne`](/new-docs/managing-data/sdk-api/overview), which returns a single document directly instead of a page.

Pagination sits at the end of the query pipeline: [filter](/new-docs/managing-data/sdk-api/filter) chooses the documents, [sort](/new-docs/managing-data/sdk-api/sort) orders them, and `limit`/`page` decide how many of that ordered set you receive at a time.
