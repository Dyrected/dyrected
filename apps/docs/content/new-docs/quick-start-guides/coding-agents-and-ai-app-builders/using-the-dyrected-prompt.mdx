...
description: Use Dyrected's staged setup prompt to connect an existing AI-built site without turning the integration into one large unreviewed change.
---

Use this page when your site already exists and you want an AI builder or coding agent to connect Dyrected in small, reviewable stages. This page is also written to be handed directly to the agent. By the end, the operator should know what the prompt is for, and the agent should know the order it is expected to follow.

The Dyrected prompt is the recommended path for AI-built websites because it does not start by installing a CMS blindly. It starts by inspecting the real site, separating content from design, and asking for approval before it changes anything major.
...
- the same AI tool can read and edit the code
- you want to decide exactly what the client will be able to edit
- you want Dyrected added without redesigning the site

This is especially useful for sites built in Lovable, Bolt, v0, Cursor, Replit, Windsurf, or similar tools.

If you are setting up Dyrected directly by hand in a self-hosted app, start with [Installation](/new-docs/basics/getting-started/installation) instead.

## Agent instructions

If you are the AI agent reading this page, follow these rules:

- inspect the real site before proposing a schema
- describe editable content in plain language first
- ask for approval before broad install work or schema creation
- preserve the current design, routes, and behavior
- work in small verified batches when the site is large
- stop and surface the real decision when a question is editorial rather than technical

If you do not have local code-editing or CLI access, route the user to the app-builder flow you can actually complete. If you do have that access, continue with the steps below.
...
- improve copy unless asked
- add new features
- remove existing features
- invent pages, sections, content types, images, routes, or product behavior
- expose layout controls when the real goal is content editing

Preserve the current design, layout, styling, routes, content order, animations, responsive behavior, and application behavior throughout the workflow.

Only ask plain-language questions. Do not ask the operator to make technical implementation decisions when the real question is about editing scope, client control, or content ownership.

## Why the prompt is staged

The safest Dyrected integration is not one big "add a CMS" request.

The prompt is designed to force a better order:

1. inspect the existing site
2. identify what content should become editable
3. ask you to approve that plan
4. connect Dyrected in batches
5. stop for a real content check before handoff

That order protects the current design and makes it easier to catch mistakes while the integration is still small.

## Before you paste it

Set up these three things first:

- create the site in Dyrected Cloud
- keep the website project open in the AI tool that owns the code
- keep your `Site ID`, `Site API key`, and `Base URL` ready

If you have not done that yet, start with [Setting Up Your Cloud Site](/new-docs/quick-start-guides/coding-agents-and-ai-app-builders/setting-up-your-cloud-site).

## Copy the prompt

Copy the cloud setup prompt from Dyrected:

<CopyPromptButton />

Paste it into the same AI builder or coding agent that can change the website code.

## Staged workflow

The prompt should guide the agent through this sequence and should not skip ahead.

### 1. Inventory the site

Inspect the actual codebase and describe the content a website owner may reasonably need to update.

At this stage, you want plain language, not schema jargon. A good response sounds like:

- homepage hero heading and button text
- team member cards
- FAQ items
- contact details
- blog posts

Do not start by defining collections, globals, or fields. Start by naming the editable content the operator is being asked to hand over.

At this stage, identify:

- visible page sections
- repeated content patterns
- repeatable business entities such as services, team members, posts, events, or FAQs
- shared site-wide settings such as navigation, footer, or contact details
- editable images and media
- long-form content that should later use rich content
- routable content that may need preview
- content-driven interactive definitions, while keeping runtime logic separate

Classify each editable area using these questions:

- is this shared site-wide or repeated?
- is this a business entity or a visible page section?
- should the editor only edit it, add and remove items, or arrange approved sections?

Stage 1 output should stay short and human-readable. Present it as a checklist of editable areas, not a schema plan.

End Stage 1 with an explicit approval stop. Do not continue until the operator confirms the list is correct.

### 2. Get approval on the editing plan

This is the most important checkpoint.

Ask what belongs in Dyrected and what should stay protected. Only proceed once the operator has approved the content handoff.

Good Dyrected content usually includes:

- page copy
- images
- lists of repeatable items such as services, team members, FAQs, or posts
- shared site settings such as navigation, footer, and contact details

Keep these out of the client editing surface unless you explicitly want them managed:

- layout and spacing
- typography and visual styling
- animations
- routing logic
- app behavior such as calculators, submissions, or scoring

When a question is really about client control, ask it that way. For example:

- should the client be able to add more of these items?
- should this stay fixed in code, or become editable content?
- should editors change only the text here, or also the list itself?

Do not talk about schemas, field types, or database structure until the operator has approved the editing boundary.

At the end of this stage, the operator should have approved:

- what content becomes editable
- what stays protected
- what editors can add or remove
- what editors can rearrange
- what should remain code-owned

Do not infer approval from silence. Wait for it explicitly.

### 3. Separate content from the UI first

Before Dyrected is connected, move the approved content into structured local data where needed.

At this stage:

- do not install Dyrected yet if safe local separation still needs to happen first
- do not connect the frontend to Dyrected yet
- do not treat one large page object as the default model
- do preserve existing behavior while making the content easier to map into Dyrected

Use simple serializable content structures. Keep runtime logic, event handlers, state, animations, and scoring logic in code.

For long-form content, keep semantic structure intact. Do not flatten article content into arrays of paragraph strings.

For images during local separation:

- preserve working image references
- keep alt text where it already exists or clearly belongs
- do not replace working assets with fake URLs
- prepare editable images to map cleanly to Dyrected media later

For section variants or icons:

- store a safe stable name in the content
- map that name to the visual component in code
- do not store JSX, components, handlers, or styling logic inside content

At the end of this stage, the site should still look and behave the same while reading from the new local content structures.

This stage should also end with an approval stop before the Dyrected install broadens the change surface.

### 4. Present the content plan

Before install work becomes broad, present the plan in plain language:

- what becomes a global
- what becomes a collection
- what becomes a reusable page section
- what editors can edit, add/remove, or arrange
- which routable content should support preview
- which media should become editable
- which existing content should seed initial data

Treat pages as ordered reusable sections by default when the visible site already works that way.

The plan should also call out:

- which content will become `Globals`
- which content will become `Collections`
- which page sections will become reusable blocks or page sections
- which collections should use a human-readable admin title
- which fields are for routing and which are for display
- which routable collections should support preview

Do not present this as raw config. Present it as the client editing plan in plain language.

### 5. Install and connect Dyrected

Only after approval should install work begin.

Depending on the project, that may include:

- running `npx dyrected init`
- adding Dyrected packages
- creating or updating `dyrected.config.ts`
- connecting approved content areas to existing frontend components
- adding Cloud credentials when the prompt asks for them

For larger sites, work in small verified batches instead of rewriting everything in one pass.

Use only APIs, field types, hooks, preview options, and package behavior supported by the installed Dyrected version and current docs.

If the docs and installed package disagree, use the installed package as the source of truth and explain the mismatch plainly.

When modeling content in this stage:

- use `Globals` for shared one-off site settings
- use `Collections` for repeatable business entities
- use reusable page sections for visible sections the operator should be able to arrange
- use rich content for long-form editorial content
- use upload/media fields for genuinely editable media

When wiring the frontend:

- make Dyrected the real source of truth for approved content
- do not leave old local content acting as the hidden normal source
- keep a fallback only where it helps testing or first-run resilience
- preserve the current route structure and cache behavior unless a change is required for CMS freshness or preview

Only add hooks when they support approved client-visible behavior. Do not add speculative hooks.

### 6. Test one real edit

Before handoff, make one content change you can recognize easily.
...
- one extra item in a repeatable list

The site should still look right after the content changes.

Stage 6 is not optional. Do not call the integration done until a real content edit has been tested end to end.

## Content architecture defaults

The rules below are generated from `@dyrected/knowledge`, so this page stays aligned with the Dyrected skill and the rest of the AI setup surface.
...
Even when content moves into Dyrected, these usually stay in code:

- component layout and styling
- animations and motion logic
- state management
- form submission handlers
- scoring or calculator logic
- authentication flows
- analytics
- user submissions and private history
- unsupported section rendering fallbacks

Dyrected should manage what the site says and which approved content pieces exist. Code should continue to control how the site works.

## Work in batches

For large sites, do not stack every change into one pass.

Work in small batches:

1. install the foundation
2. connect one small group of related content areas
3. verify the site still works
4. confirm one CMS edit appears correctly
5. only then move to the next group

If a batch fails verification, stop and fix that batch before continuing.

For each batch, verify:

- lint, typecheck, tests, or build commands that exist in the project
- one real content read path from Dyrected into the frontend
- no accidental design regression in the touched area
- no private credentials leaking into browser code

If the project does not have one of those commands, say so plainly and continue with the checks that do exist.

## Final verification

Before handoff, confirm all of the following:

- the existing design still looks the same
- approved content is editable and protected content is still protected
- pages or routable collections use the right title and slug roles
- preview works for the collections that need it
- media and alt text still render safely
- rich content preserves headings, paragraphs, lists, links, and formatting
- frontend routes now read Dyrected where they should
- local fallback content is not still acting as the hidden source of truth
- one real Dyrected edit appears on the website

The final handoff summary should explain in plain language:

- what the client can now edit
- what they can add or remove
- which page sections they can reuse or arrange
- which content remains protected
- where they sign in
- how to test one safe edit before publishing

Do not hand off a site that has not passed a real content edit test.

## When to steer harder

Step in immediately if the agent:

- starts redesigning the site instead of separating content from presentation
- proposes one giant page object where structured fields would be clearer
- tries to expose layout controls to editors
- keeps adding new content areas before the current batch is verified
- asks technical questions that are really editorial decisions

When that happens, bring it back to the real decision. For example:

- "Keep the current design. Only make the text and images editable."
- "Treat these as repeatable items, not one large rich text field."
- "Do not continue until one edit appears on the page correctly."

## What belongs on this page

This page is the canonical guide for using the staged prompt workflow and for telling the agent how to behave during that workflow.

Use it for:

- deciding whether the prompt is the right path
- instructing the agent on the approval loop
- understanding what kinds of content usually belong in Dyrected
- keeping the integration bounded

Use [Add Dyrected skill to your agent](/new-docs/quick-start-guides/coding-agents-and-ai-app-builders/add-dyrected-skill-to-your-agent) when you are working in a local agent runtime that supports installable skills and you want Dyrected knowledge loaded on demand.

## Success check

This workflow is going well when:

- the agent describes editable content in plain language first
- the client editing plan is approved before install work starts
- the integration happens in bounded batches
- one Dyrected edit appears on the site before editor handoff

If you prefer an active agent skill rather than a pasted prompt, continue to [Add Dyrected skill to your agent](/new-docs/quick-start-guides/coding-agents-and-ai-app-builders/add-dyrected-skill-to-your-agent).
