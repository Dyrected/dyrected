...
A Jexl string is a restricted expression — it looks like JavaScript comparison syntax but cannot run arbitrary code, which is what keeps it safe to store and sync. It can return a boolean or, for row-level access, an object:

```ts
access: {
  read: "'admin' in user.roles",                 // boolean result
  update: "{ owner: { equals: user.sub } }",      // filter result — see Collections
}
```
