...
- `name` — the persisted key. Treat renames as data migrations.
- `label` — the human-readable name shown to editors.
- `required` — whether the field must have a value.
- `defaultValue` — the value used when none is provided.
- `unique` and `index` — storage constraints on relational adapters.
- `admin` — per-field admin options such as descriptions and conditional display.
