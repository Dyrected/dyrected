...
Each row stores the chosen block's `slug` as `blockType`, so your frontend can map each `blockType` to a component and render the list in order. Keep block `slug` values stable once content exists — renaming a slug orphans the rows already saved under the old name — and have your renderer fall back gracefully when it meets a `blockType` it does not recognize.
