...
Generate a TypeScript types file from your schema. With no flags it reads `./dyrected.config.ts` and writes `./dyrected-types.ts`. Point it at a different config, a running site, or a different output path with the flags below.
...
| `-o, --output <path>` | Where to write the generated file. Defaults to `./dyrected-types.ts`. |
...
If your API key or Site ID is missing, `sync:schema` skips quietly rather than failing — so it's safe to leave in a self-hosted build. One caveat worth knowing: Cloud can't run **function-based** access rules, so the sync strips them and warns you which paths were dropped. Replace any function access rule you rely on with a Jexl string or a named policy before trusting Cloud to enforce it.
...
To go further — installing the Dyrected agent skill and getting the most out of an AI assistant — see [Coding agents and AI app builders](/new-docs/quick-start-guides/coding-agents-and-ai-app-builders/add-dyrected-skill-to-your-agent).
...
- [Generating Types](/new-docs/features/typescript/generating-types) — the full type-generation workflow and what the generated file contains.
- [Configuration Overview](/new-docs/basics/configuration/overview) — what the `dyrected.config.ts` that these commands act on actually holds.
- [Production deployment](/new-docs/deployment/production/deployment) — where `sync:schema` fits into a Cloud deploy.
