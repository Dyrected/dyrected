...
For the full field system, read [Fields](/new-docs/basics/fields/overview).
...
For more on page sections, read [Blocks](/new-docs/basics/fields/blocks).
...
| <code>access</code> (optional) | Collection-level access control.  Each key is an operation; the value can be a function, a Jexl string, a boolean, or a named policy reference. Returning &#96;true&#96; allows access and &#96;false&#96; denies it. Returning a &#96;where&#96;-style object grants access only to matching documents.  See: [Access control overview](https://dyrected.com/new-docs/basics/access-control/overview) |
| <code>hooks</code> (optional) | Collection-level lifecycle hooks.  Hooks run in the order they appear in the array. The return value of each hook is passed as the input to the next. Throwing inside any hook aborts the operation and returns a &#96;500&#96; error.  See the Hooks reference for the full lifecycle diagram.  See: [Hooks overview](https://dyrected.com/new-docs/basics/hooks/overview), [Collection hooks](https://dyrected.com/new-docs/basics/hooks/collections) |
| <code>admin</code> (optional) | Admin UI configuration for this collection.  See: [Admin options](https://dyrected.com/new-docs/basics/configuration/collections#admin-options) |
...
| Option           | What it does                                                                                                                                                                                                                                                                                                                                         |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `useAsTitle`     | Chooses which field Dyrected should treat as the human-readable name for each document in list views, breadcrumbs, relationship pickers, and other admin surfaces. Use the field editors recognize fastest, usually something like `title`, `name`, or `email`.                                                                                      |
...
See [Access Control Overview](/new-docs/basics/access-control/overview) for the full model.
...
See [Hooks Overview](/new-docs/basics/hooks/overview) for the lifecycle and [Collection Hooks](/new-docs/basics/hooks/collections) for collection-focused examples.
...
For the full auth model, token behavior, and login strategies, read [Authentication Overview](/new-docs/features/authentication/overview).
...
For storage behavior and adapter-level upload setup, read [Upload Overview](/new-docs/features/upload/overview). For the field-level side of file relationships, read [Upload Field](/new-docs/basics/fields/image).
...
This is a larger feature with its own states, transitions, and roles, so it lives on its own page. For the full model, read [Workflows Overview](/new-docs/features/workflows/overview).
...
For what gets recorded and how it works, read [Audit Overview](/new-docs/features/audit/overview).
...
For field-level modeling, read [Fields](/new-docs/basics/fields/overview).

Hooks are optional here. You only need them if you want Dyrected to derive routable values automatically, such as generating a document slug from a title before save. For that workflow, read [Hooks Overview](/new-docs/basics/hooks/overview).
...
- See [Globals](/new-docs/basics/configuration/globals) for singleton content.
- See [Overview](/new-docs/basics/configuration/overview) for the full root config shape.
- See [Fields](/new-docs/basics/fields/overview) for the schema system that powers collection documents.
