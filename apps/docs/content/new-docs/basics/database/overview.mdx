...
- [Postgres](/new-docs/basics/database/postgres) for most relational deployments.
- [SQLite](/new-docs/basics/database/sqlite) for local development and small single-node apps.
...
The contracts below are generated from the public `@dyrected/core` exports by `@dyrected/knowledge`, so the adapter interfaces stay in sync with the package. Use them when writing a custom adapter or when you need the exact shape of a paginated result.
