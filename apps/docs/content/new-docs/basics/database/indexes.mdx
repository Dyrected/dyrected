...
```ts
{ name: 'slug', type: 'text', label: 'Slug', unique: true, promoted: true }
```
...
So if you depend on uniqueness, enforce it in your own logic — for example, a `beforeChange` hook that looks for an existing document with that value and rejects the write if it finds one. See [Collection hooks](/new-docs/basics/hooks/collections).

## MongoDB

MongoDB stores every field natively, so there's nothing to promote — `promoted` has no effect there. Query performance instead comes from MongoDB's own indexes, which Dyrected does not create for you. Add the indexes you need directly in MongoDB. See [MongoDB](/new-docs/basics/database/mongodb) for the details.
...
For the broader performance picture — pagination, query shape, and depth — see [Performance](/new-docs/deployment/performance/overview) and the [query APIs](/new-docs/managing-data/sdk-api/filter).
