...
Because the document body lives in JSONB, you can filter and sort on any field without declaring it up front. Fields you mark with `promoted: true` are also lifted into their own real, typed columns so the database can work with them directly. That is the mechanism to reach for on fields you query often. See [Indexes](/new-docs/basics/database/indexes) for when and how to promote a field.

## Transactions and raw SQL

The Postgres adapter runs multi-step writes as real database transactions, and workflow transitions rely on this. Reads issued inside a transaction take row locks, so concurrent writers wait their turn — that makes read-modify-write safe. See [Transactions](/new-docs/basics/database/transactions) for how to group operations yourself.
...
- Startup schema sync is additive: it creates missing tables and adds promoted columns, but it does not drop or retype existing ones. Back up before a schema change and roll it out carefully. See [Migrations](/new-docs/basics/database/migrations) and [Deployment](/new-docs/deployment/production/deployment).
