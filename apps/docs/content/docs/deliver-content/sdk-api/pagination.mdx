...
If you already have the document's ID, skip pagination entirely and use [`findOne`](/docs/deliver-content/sdk-api/overview), which returns a single document directly instead of a page.

Pagination sits at the end of the query pipeline: [filter](/docs/deliver-content/sdk-api/filter) chooses the documents, [sort](/docs/deliver-content/sdk-api/sort) orders them, and `limit`/`page` decide how many of that ordered set you receive at a time.
