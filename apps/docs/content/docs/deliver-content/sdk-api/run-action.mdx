...
For row actions the method returns the updated document. For bulk actions it returns `{ modified: number }`. For header actions it returns the result payload from your handler, defaulting to `{ success: true }` when no custom payload is returned. In all cases the type is `UnknownRecord` until you narrow it with your `DyrectedSchema`.
...
| `args.id` | `string` | Target document ID for row actions (omit for header actions). |
| `args.ids` | `string[]` | Target document IDs for bulk actions (omit for header actions). |
