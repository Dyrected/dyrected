...
Most JavaScript and TypeScript apps should use the [SDK](/docs/deliver-content/sdk-api/overview) instead of calling REST directly. Reach for REST when you need a language the SDK does not cover, or when you are integrating from another system.
...
Aggregations enforce collection `access.read` gates and inject any row-level tenant constraints automatically. For detailed syntax and type casting, see [Aggregating Statistics](/docs/deliver-content/sdk-api/aggregate).

## Media and uploads

Upload-enabled collections expose dedicated routes for file ingestion and atomic in-place replacement:

### Upload a new file

Send a `multipart/form-data` request with the `file` field and optional document fields:

```bash
curl -X POST https://example.com/api/collections/media/upload \
  -H "x-api-key: your-api-key" \
  -F "file=@/path/to/hero.jpg" \
  -F "alt=Hero campaign banner" \
  -F "folderId=marketing-folder-id"
```

### Replace an asset in-place

Replace the physical file bytes for an existing asset without changing its document ID or breaking relations:

```bash
curl -X POST https://example.com/api/collections/media/asset-123/file \
  -H "x-api-key: your-api-key" \
  -F "file=@/path/to/new-hero.jpg"
```

Dyrected replaces the stored file in your storage adapter, recalculates dimensions, aspect ratio, and BlurHash, and returns the updated document.

## Media folders

Organize upload collections into nested hierarchical folders:

```bash
GET /api/collections/:slug/folders
POST /api/collections/:slug/folders
PATCH /api/collections/:slug/folders/:id
DELETE /api/collections/:slug/folders/:id
```

### Create a folder

```bash
curl -X POST https://example.com/api/collections/media/folders \
  -H "Content-Type: application/json" \
  -H "x-api-key: your-api-key" \
  -d '{
    "name": "Summer 2026",
...
Delete multiple assets in a single atomic request:

```bash
curl -X DELETE https://example.com/api/collections/media/delete-many \
  -H "Content-Type: application/json" \
  -H "x-api-key: your-api-key" \
  -d '{
...
Regenerate whenever the content contract changes. Do not treat an old downloaded specification as permanently compatible. For JavaScript and TypeScript, the [SDK](/docs/deliver-content/sdk-api/overview) already gives you a typed client with no code generation step.
...
| --- | --- | --- | --- |
| GET | `/api/ai/actions/{actionId}` | Get an AI mutation action proposal | Required |
| POST | `/api/ai/actions/{actionId}/execute` | Approve and execute an AI mutation proposal | Required |
| POST | `/api/ai/actions/{actionId}/reject` | Reject an AI mutation proposal | Required |
| POST | `/api/ai/chat` | Send prompt and stream AI assistant response | Required |
| POST | `/api/ai/rag/reindex` | Trigger vector reindexing for semantic search | Required |
| POST | `/api/ai/rag/search` | Perform semantic vector search against indexed content | Required |
| DELETE | `/api/ai/threads` | Clear all conversation threads | Required |
| GET | `/api/ai/threads` | List conversation threads | Required |
| POST | `/api/ai/threads` | Create a new conversation thread | Required |
| DELETE | `/api/ai/threads/{threadId}` | Delete a specific conversation thread | Required |
| GET | `/api/ai/threads/{threadId}` | Get conversation thread and historical messages | Required |
| POST | `/api/ai/threads/{threadId}/messages` | Post a message to an existing thread and stream reply | Required |
...
| GET | `/api/collections/users/tokens/verify` | Verify an invite or password reset token | Required |
