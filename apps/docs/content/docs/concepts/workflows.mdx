...
3. **Public vs. Authenticated Views**: Anonymous visitors calling `client.find()` or `client.findOne()` are served the fields from `__published` directly. Authenticated users (e.g. in the Admin UI) see the current working draft fields so they can continue editing without affecting the live site.
