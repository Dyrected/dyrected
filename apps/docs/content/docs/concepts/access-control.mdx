...
<Accordion title="Blog: public reads, staff writes, authors own their content">
...
export const Posts = defineCollection({
  ...
  access: {
    ...
    create: 'user != null',
    update: "user.role == 'admin' || user.sub == doc.authorId",
    delete: "user.role == 'admin'",
  },
  fields: [...],
})
...
<Accordion title="Contact form / lead capture: public submit, staff reads only">
...
<Accordion title="E-commerce orders: users see their own, admins see all">
...
export const Orders = defineCollection({
  ...
  access: {
    read: ({ user }) => {
      if (!user) return false
      if (user.role === 'admin') return true
      return { customer: { equals: user.sub } }  // row-level filter
    },
    create: ({ user }) => !!user,
    update: ({ user }) => user?.role === 'admin',
    delete: ({ user }) => user?.role === 'admin',
...
<Accordion title="Role-based team: admin / editor / viewer">
...
<Accordion title="Invite-only community: restrict create to admins">
...
export const Members = defineCollection({
  ...
  access: {
    ...
    update: ({ user, doc }) => user?.role === 'admin' || user?.sub === doc?.id,
    delete: () => false,
  },
  fields: [...],
})
