...
The Dyrected engine is an npm package that runs **inside your Next.js or Nuxt.js app**. There is no separate CMS process. Your config, your database, your server.
...
Yes. Both modes use the same `dyrected.config.ts`. To migrate from self-hosted to Cloud: export your data, create a site in the Cloud dashboard, run `npx @dyrected/cli sync:schema` to sync your schema, and update your environment variables. No code changes needed.
