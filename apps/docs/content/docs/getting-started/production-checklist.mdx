...
## 4. Boolean Filter Tip

For SQL databases (PostgreSQL/MySQL), Boolean fields are extracted from JSON storage structures as text data types. 

- **Recommendation:** Although the latest versions of the framework parse booleans seamlessly, as a general best practice for SQL backends, ensure you are testing query filters involving booleans (`attending: { equals: true }`) in staging before promoting to production.
