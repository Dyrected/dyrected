...
When running `@dyrected/core` directly, the Admin UI is served at `/admin` automatically. No embedding needed.
...
- **Columns:** Controlled by `admin.defaultColumns`. Falls back to the first three non-hidden fields. `row` and `join` virtual fields are excluded from columns automatically.
