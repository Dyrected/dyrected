...
| `layout` | field-specific | field default | Presentation style only on fields that support alternate renderers. Boolean fields accept `checkbox` or `switch`. Select fields accept `select` or `radio`. |
