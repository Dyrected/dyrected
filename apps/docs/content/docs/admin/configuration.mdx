...
Components receive the same props as the dashboard, plus:
- `collection`: The configuration of the current collection.
- `data`: The paginated list response (`{ docs, totalDocs, totalPages, ... }`).
- `page` / `setPage`: Current pagination state.
- `where` / `setWhere`: Current filter state.
- `selectedIds` / `setSelectedIds`: Array of document IDs selected via table checkboxes.

If an unconfigured component is passed, or if a component throws an error during render, the Admin UI will gracefully catch the error and skip it, ensuring the rest of the CMS remains usable.
