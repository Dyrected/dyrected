...
<Callout type="warn">
  **Dyrected Cloud Sync limitation** If you use Dyrected Cloud Sync, your schema is serialized to JSON before being sent
  to the cloud. Because JSON cannot serialize JavaScript functions, any function passed to `previewUrl` will be
  stringified and break the Admin UI. Cloud users **must** use Jexl strings.
</Callout>
