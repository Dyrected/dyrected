...
`logout` is stateless. Because the token _is_ the session, there is nothing to delete on the server — the endpoint just confirms and reminds you to drop the token client-side:
