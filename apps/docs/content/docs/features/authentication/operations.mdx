...
// → { success: true, message, token, inviteUrl }
```

When you pass `inviteUrl`, Dyrected emails a clickable acceptance link in the same style as password reset emails. Without it, the default template falls back to the raw invite token.
