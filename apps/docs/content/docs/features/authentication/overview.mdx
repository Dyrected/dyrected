...
Use [Handing Off to Editors](/docs/features/authentication/handing-off-to-editors) for the full onboarding checklist.
...
- [Handing Off to Editors](/docs/features/authentication/handing-off-to-editors) — invite people into the right editing path
- [Cloud vs Self-hosted](/docs/basics/getting-started/cloud-vs-self-hosted) — compare runtime boundaries
- [Cloud-safe access control](/docs/basics/access-control/cloud) — write content rules that run in Cloud
...
Dyrected also mounts auth endpoints under `/api/collections/{slug}/`: `login`, `logout`, `me`, `refresh-token`, `forgot-password`, `reset-password`, `invite`, `accept-invite`, `init`, and `first-user`. Each request and response shape is documented in [Operations](/docs/features/authentication/operations).
...
The SDK holds the token in memory only. Deciding where the token lives between requests is the job of the [Cookie Strategy](/docs/features/authentication/cookie-strategy) page.
...
The full walkthrough lives in [Handing Off to Editors](/docs/features/authentication/handing-off-to-editors).
...
This is separate from collection auth. It only governs who reaches the CMS and uses routes under `/api/admin/auth/`. If your team needs single sign-on for the admin panel, see [Admin SSO](/docs/features/authentication/admin-sso).
...
- [Admin SSO](/docs/features/authentication/admin-sso) — external dashboard sign-in
