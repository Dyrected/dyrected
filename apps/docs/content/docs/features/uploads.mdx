...
Setting `upload: true` on a collection enables file uploads. Dyrected adds storage integration, image resizing, MIME-type validation, and a dedicated upload browser in the Admin UI automatically. The collection does not need to be named `media`; labels from your upload collection are used throughout the Admin UI.
...
```ts
upload: {
  uploadDir: './public/media',       // Local disk path to store files (LocalStorageAdapter only)
  staticUrlPrefix: '/media',         // Public URL prefix for locally stored files
...
In the Admin UI, upload collections appear in a responsive grid. The Media Library and form-level Media Picker share the same selection language, and the attachment detail dialog is scrollable on small screens so large previews and metadata stay inside the viewport.
...
| Adapter | `url` format |
|---|---|
| `LocalStorageAdapter` | `/uploads/filename.jpg` (relative, based on `staticUrlPrefix`) |
| `S3StorageAdapter` | `https://bucket.s3.region.amazonaws.com/filename.jpg` |
| `CloudinaryStorageAdapter` | `https://res.cloudinary.com/cloud-name/image/upload/filename.jpg` |
