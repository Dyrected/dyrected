...
export async function GET(req: Request) {
  ...
  if (!previewData) return new Response('Invalid token', { status: 401 })
...
Inline editing lets content editors click any element in the preview iframe to jump directly to the corresponding field in the Admin sidebar — no more hunting for the right input.

### How it works

1. **Admin enters Edit Mode** — the editor clicks the **Edit** button in the preview toolbar. The Admin sends `dyrected-enter-edit-mode` to the iframe.
2. **Frontend adds click listeners** — on receiving the message, your page attaches hover highlighting and click handlers to every element that has a `data-dy-path` attribute.
3. **Editor clicks an element** — the iframe sends `dyrected-element-clicked` (with the field path) back to the Admin.
4. **Admin scrolls to field** — the Admin finds the matching form field by `data-dy-field` and scrolls + focuses it.

### Marking elements (Vue / Nuxt)

Add `data-dy-path` to any element whose value maps to a field name:
...
The `useLivePreview` composable handles the rest automatically — no extra setup needed.

### Marking elements (React / Next.js)

```tsx
export default function PostPreview({ post }: { post: Post }) {
  const { data } = useLivePreview<Post>({ initialData: post, serverURL: '...' })

  return (
    <article>
      <h1 data-dy-path="title">{data.title}</h1>
      <p data-dy-path="excerpt">{data.excerpt}</p>
    </article>
  )
}
```
...
The value of `data-dy-path` must match the field `name` as defined in the collection config. For nested fields use dot notation:

```html
<span data-dy-path="seo.metaTitle">{{ post.seo.metaTitle }}</span>
```

> **Tip:** You don't need to add `data-dy-path` to every element — only the ones where clicking would be useful. Headings, images, and key text fields are the highest-value targets.
