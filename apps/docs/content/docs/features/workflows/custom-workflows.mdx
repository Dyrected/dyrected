...
The five recipes below are complete and copy-paste ready — each a real-world pattern that swaps in a different state machine. The surrounding collection and the way you drive it (see [Performing a transition](/docs/features/workflows/overview)) never change.
...
The "at the right time" part lives outside the workflow: a job compares each `scheduled` document's embargo field to the clock and calls the `publish` transition when it's due. Wire that up by reacting to workflow changes — see [Lifecycle events](/docs/features/workflows/lifecycle-events).
...
For the complete list of every `WorkflowConfig` field, see the reference at the bottom of the [Editorial workflows overview](/docs/features/workflows/overview).
