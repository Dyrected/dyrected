...
| Reset, invite, welcome, or password-changed emails never arrive     | Email transport is unset or only development fallback is active     | [Auth emails are not sending](#auth-emails-are-not-sending)             |
...
- the self-hosted admin invite flow still produces a copyable invite URL, so you can confirm link generation separately from provider delivery
