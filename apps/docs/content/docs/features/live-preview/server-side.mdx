...
<Callout type="info">
  ...
  [client-side (`postMessage`)](/docs/features/live-preview/client-side) instead.
</Callout>

## Turn it on

Set `previewMode: "token"` on the collection, alongside the `previewUrl` you already configured (see [Preview](/docs/features/admin/preview)):
...
- [Client-side integration](/docs/features/live-preview/client-side) — the `postMessage` path, with live-as-you-type and click-to-edit.
- [Preview configuration](/docs/features/admin/preview) — `previewUrl` and `previewMode` reference.
