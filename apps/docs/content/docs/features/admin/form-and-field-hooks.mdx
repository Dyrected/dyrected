...
### String Helpers
- `slugify(str)`: Transforms text into a clean URL-safe slug (`slugify(siblingData.title)` $\to$ `"hello-world"`).
- `lower(str)` / `upper(str)`: Converts string casing.
- `trim(str)` / `capitalize(str)`: Removes surrounding whitespace or capitalizes the first word.
