...
The date field stores a calendar day and shows editors a date picker. Dates cross the API as ISO strings, so parse them at your application boundary when you need to do date arithmetic. When you also need a time, reach for [Datetime](/docs/basics/fields/datetime); for a time of day on its own, use [Time](/docs/basics/fields/time).

```ts
defineDateField({ name: 'publishedAt', label: 'Published at' })
```

Dates are often sorted and filtered on — a `publishedAt` you order posts by, for instance. If you query a date field often, mark it `promoted` so it becomes an indexed column. See [Database indexes](/docs/basics/database/indexes).
