...
- [Postgres](/docs/basics/database/postgres) for most relational deployments, and the recommended production default.
- [SQLite](/docs/basics/database/sqlite) for local development and small single-server apps.
- [MongoDB](/docs/basics/database/mongodb) for document-oriented storage.
- [MySQL](/docs/basics/database/mysql) for MySQL deployments; it behaves like the Postgres adapter.
...
On the relational adapters (Postgres, SQLite, MySQL), each collection is a table named `collection_<slug>`. A document is stored as one JSON column, `data`, alongside `id`, `created_at`, and `updated_at`. Because the body is JSON, you can filter and sort on any field without declaring it first. Fields you mark [`promoted`](/docs/basics/database/indexes) are also lifted into their own real columns for faster queries. Globals live in a small internal table.
...
- [Migrations](/docs/basics/database/migrations) — how schema changes are applied, and how to rename fields safely.
- [Transactions](/docs/basics/database/transactions) — grouping writes so they all commit or all roll back.
- [Indexes](/docs/basics/database/indexes) — promoting the fields you query for speed, and how uniqueness fits in.
