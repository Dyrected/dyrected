...
This page is the model: what a rule is, the shapes it can take, when it runs, and what it can see. The pages that follow show where to put rules for [collections](/docs/basics/access-control/collections), [globals](/docs/basics/access-control/globals), and [fields](/docs/basics/access-control/fields). If you are deploying to Dyrected Cloud, also read [Dyrected Cloud access control](/docs/basics/access-control/cloud) for the Cloud-specific policy limits and built-in policy names.
...
- An **object** result allows the operation but scopes it to matching documents — a row-level filter, covered in [Collections](/docs/basics/access-control/collections#document-aware-rules).
...
If you need a policy with parameters, use one of the Cloud built-ins. For the current built-in policy list and common parameters, see [Dyrected Cloud access control](/docs/basics/access-control/cloud#built-in-named-policies-in-dyrected-cloud).
...
The scaffolded auth collection ships with a `roles` field offering `admin`, `editor`, and `viewer`, which is why `'admin' in user.roles` is the idiom you will see most often. For the full picture of what lands on `user`, see [Token data](/docs/features/authentication/token-data).
...
|---|---|---|
| [Collections](/docs/basics/access-control/collections) | `read`, `create`, `update`, `delete`, `readAudit` | Whole documents in a collection, plus the audit log |
| [Globals](/docs/basics/access-control/globals) | `read`, `update` | A single global document |
| [Fields](/docs/basics/access-control/fields) | `read`, `create`, `update` | Individual fields — stripped from responses or dropped on writes |
...
- [Collection access control](/docs/basics/access-control/collections) — the four operations, row-level filtering, and common recipes.
- [Global access control](/docs/basics/access-control/globals) — read and update rules for single-document config.
- [Field access control](/docs/basics/access-control/fields) — hiding and locking individual fields.
- [Hooks](/docs/basics/hooks/overview) — for logic that needs to transform data or raise custom errors, not just allow or deny.
