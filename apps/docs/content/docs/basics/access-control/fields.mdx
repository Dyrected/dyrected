...
So field access is a real server-side boundary, not just a display hint. Read the [Access Control overview](/docs/basics/access-control/overview) for the shared model, then use this page for the field-specific behavior.
...
Field rules are evaluated with the standard [access context](/docs/basics/access-control/overview#what-a-rule-can-see), plus the form's live values when they run in the admin panel:
...
Rules apply at every level of nesting. A `read` or `update` rule on a field inside a [group](/docs/basics/fields/overview), array, or blocks field is enforced on that nested value too.
...
Field access answers a yes/no question: can this field be read or written? Reach for a [field hook](/docs/basics/hooks/fields) when you need to do more than allow or deny:
...
- [Collection access control](/docs/basics/access-control/collections) — whole-document rules.
- [Field configuration](/docs/basics/fields/overview) — the options every field accepts.
- [Field hooks](/docs/basics/hooks/fields) — server-side logic per field.
