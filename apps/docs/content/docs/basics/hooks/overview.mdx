...
## Cloud-safe declarative hooks

If you are deploying to Dyrected Cloud, there is a smaller Cloud-safe subset of hooks that can be serialized into schema and evaluated without arbitrary server code.

The short version:

- use **declarative string hooks** for simple read shaping and value transforms that should survive `sync:schema`
