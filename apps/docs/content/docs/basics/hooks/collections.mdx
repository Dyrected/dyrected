...
For Cloud-safe `beforeRead` details, including the exact expression context, see [Cloud-safe hooks](/docs/basics/hooks/cloud).

<Note>
Reach for [access control](/docs/basics/access-control/collections) first when the question is who is allowed to see a document at all. Reach for `beforeRead` when the question is how an already-allowed read should be shaped or narrowed at the hook layer.
</Note>
...
For Cloud-safe `afterRead` details, including merge behavior for object returns, see [Cloud-safe hooks](/docs/basics/hooks/cloud).
...
For Cloud-safe `beforeChange` details, including the exact expression context and merge behavior, see [Cloud-safe hooks](/docs/basics/hooks/cloud).
...
- Keep access decisions in [access control](/docs/basics/access-control/collections) when the question is allow or deny. Use hooks when the question is transform, validate, react, or reshape an already-authorized operation.

## Related pages

- [Hooks overview](/docs/basics/hooks/overview) for the full map of hook families.
- [Cloud-safe hooks](/docs/basics/hooks/cloud) for the Cloud-specific subset and sync behavior.
- [Global hooks](/docs/basics/hooks/globals) for the singleton lifecycle.
- [Field hooks](/docs/basics/hooks/fields) for value-level transforms.
- [Hook context](/docs/basics/hooks/context) for the `req`, `user`, and `db` arguments these hooks receive.
