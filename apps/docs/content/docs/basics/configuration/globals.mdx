...
Because a global is one document, think of `fields` as the shape of that single record rather than a template for many. For the full field system, read [Fields](/docs/basics/fields/overview).
...
Reach for this whenever a global holds settings only certain roles should touch, such as letting anyone read navigation while only editors change it. For the full model, see [Access Control](/docs/basics/access-control/overview).
...
Use `update` to keep shared settings in trusted hands, for example letting any signed-in user read navigation while only editors change it. See [Access Control](/docs/basics/access-control/overview) for the full model.
...
The most common use is an `afterChange` hook that revalidates cached pages when settings change, as shown above. Errors in `after*` hooks are isolated, so a failed revalidation never turns a successful save into an error. See [Hooks](/docs/basics/hooks/overview) for the full lifecycle.
...
See [Collections](/docs/basics/configuration/collections) for the repeatable side of the model.
...
- See [Collections](/docs/basics/configuration/collections) for repeatable content.
- See [Overview](/docs/basics/configuration/overview) for the full root config shape.
- See [Environment Variables](/docs/basics/configuration/environment-variables) before wiring SDK access and secrets.
