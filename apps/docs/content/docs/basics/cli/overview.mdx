...
If your API key or Site ID is missing, `sync:schema` skips quietly rather than failing — so it's safe to leave in a self-hosted build. One caveat worth knowing: Cloud can't run **function-based** access rules or **function-based access policies**, so the sync strips them and warns you which paths were dropped. String and boolean `accessPolicies` now sync to Cloud as part of the site schema. Replace any function access rule or function policy you rely on with a Jexl string, a boolean policy, or another serializable form before trusting Cloud to enforce it.
