...
Every non-Nuxt-specific export (including `<DyrectedImage>`) is also available from `@dyrected/vue` — see the [Vue integration](/docs/integrations/vue#rendering-components) for the full component reference.
