...
Dyrected integrates natively with Next.js via a catch-all API route. Supports both App Router and Pages Router. You can fetch data server-side, client-side, or use the SDK in Server Components.

> **Recommended:** Use the CLI to set up Dyrected automatically. It detects App Router vs Pages Router, your `src/` layout, installs dependencies, writes all required files, and generates `instrumentation.ts` to log URLs on startup.
> ```bash
...
```ts
// pages/api/dyrected/[...route].ts  (or src/pages/api/dyrected/[...route].ts)
export { default } from '@dyrected/next'
```

All Dyrected REST endpoints are available at `/api/dyrected/...`.
...
If you used `npx @dyrected/cli init`, this file is already written in the correct location — the CLI detects your `src/` layout and whether you're using App Router or Pages Router.
...
```tsx
// pages/cms.tsx  (or src/pages/cms.tsx)
import { DyrectedAdmin } from '@dyrected/next/admin'
...
```
  ➜  Dyrected admin:  http://localhost:3000/cms
