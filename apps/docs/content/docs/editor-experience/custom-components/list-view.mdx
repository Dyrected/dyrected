...
The collection list is where editors browse, filter, and act on records. You can't rebuild it, but you can add your own components around it: a header with a custom action, a banner above the table, a summary row or custom pagination below. Dyrected gives you four slots on every collection list for this — now aliased as **operational view slots** so the same content mounts inside `table`/`kanban`/`calendar`/`cards`/`spreadsheet` workspaces. See [Routing and preferences](/docs/model-content/operational-views/routing-and-preferences) for the mapping.

By the end of this page you should know the four list slots (and their view aliases), the rich set of props your slot components receive, and how to register them per collection. This page assumes the declare-then-provide pattern from the [Custom Components overview](/docs/editor-experience/custom-components/overview).
...
## View-specific component slots (`defineView`)

If a collection has multiple operational views (such as a Kanban pipeline, a Calendar schedule, and a standard Table), you might only want a custom widget on a **single view** rather than all of them.

You can declare component slots directly inside [`defineView`](/docs/model-content/operational-views/define-view):

```ts
import { defineView } from "@dyrected/core";

export const kanbanPipeline = defineView({
  slug: "pipeline",
  label: "Pipeline",
  layout: "kanban",
  groupBy: "status",
  components: {
    beforeViewHeader: ["kanban-legend"],
    afterViewHeader: ["pipeline-kpi-summary"],
    beforeViewContent: ["kanban-filter-banner"],
    afterViewContent: ["kanban-footer-stats"],
  },
});
```

When both collection-wide slots (`collection.admin.components`) and view-specific slots (`view.components`) are defined, Dyrected cleanly merges them in hierarchical order (collection banners render first, followed by view-specific widgets). Register the components under `components.collectionView` or `components.collectionList` in `<DyrectedAdmin />`.

### Collection-wide vs. View-specific slots

| Scope | Where to declare | Use case |
| --- | --- | --- |
| **Collection-wide** | `collection.admin.components` | Notices, banners, or actions that should appear across **all views** in this collection (e.g. an "Archived collection" banner). |
| **View-specific** | `defineView({ components: { ... } })` | Widgets that **only make sense for a single view layout** (e.g. a Kanban legend, a Calendar date switcher). |
...
In a Vue or Nuxt app, register Vue components in the same `collectionList` map; they're wrapped automatically and receive the props above as Vue props. See the [overview](/docs/editor-experience/custom-components/overview#using-vue-components).
