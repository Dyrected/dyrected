...
Click-to-edit lets an editor click an element in the preview and have the form focus the field behind it. Call `useDyPath()` once at the top of your component to get a path annotator helper (`dy`), then call `dy('fieldName')` directly on your elements:

```tsx
// React / Next.js
import { useDyPath } from "@dyrected/react";

function Hero({ heading, subheading, cta, items }: any) {
  // Call useDyPath() once at the top of the component
  const dy = useDyPath();

  return (
    <section>
      <h1 {...dy("heading")}>{heading}</h1>
      {subheading && <p {...dy("subheading")}>{subheading}</p>}
      {cta && <button {...dy("cta")}>{cta}</button>}

      {/* Fully hook-safe in loops & nested maps */}
      <ul>
        {items?.map((item: any, idx: number) => (
          <li key={idx} {...dy(`items.${idx}.title`)}>
            {item.title}
          </li>
        ))}
      </ul>
...
<template>
  <section>
    ...
    <button v-if="cta" v-bind="dy('cta')">{{ cta }}</button>
