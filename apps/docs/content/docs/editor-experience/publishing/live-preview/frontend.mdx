...
| --- | --- | --- |
| Next.js server files | `@dyrected/next/server` | No |
| Next.js Client Components | `@dyrected/react` | No |
...
Set the mode with `previewMode` on the collection (see [Preview](/docs/editor-experience/preview)); it defaults to `postMessage`.

<Callout type="info">
  In Next.js, keep the imports split by runtime. Server Components and route handlers can use `@dyrected/next/server`.
  Client Components should import `useLivePreview`, `useDyPath`, and `Blocks` from `@dyrected/react` so the browser
  bundle does not pull in server-only handler code.
</Callout>

## Next

- [Client-side integration](/docs/editor-experience/publishing/live-preview/client-side) — the `postMessage` path: `useLivePreview`, `useDyPath`, and `<Blocks>`, with runnable React, Next.js, Vue, and Nuxt code.
- [Server-side integration](/docs/editor-experience/publishing/live-preview/server-side) — the `token` path for server-rendered and statically generated frontends.
