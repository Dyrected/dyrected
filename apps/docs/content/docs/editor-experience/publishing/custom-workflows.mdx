...
## Running logic inside a transition

Sometimes a transition has to do real work: reserve funds when a refund is approved, or release them when it is rejected. Give the transition an `onTransition` handler. It runs inside the transition's database transaction, so anything you write through `tx` commits together with the state change, and a throw rolls all of it back and leaves the document where it was.
...
The handler receives the locked `doc`, the acting `user`, the `comment`, the `req`, and `input`, an optional object the caller sends with the transition request (`{ "input": { ... } }` in the request body). Keep the work to database reads and writes; anything slow or external belongs in an `afterTransition` hook, which runs after the commit.
