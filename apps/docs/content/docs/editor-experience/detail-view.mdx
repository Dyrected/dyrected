...
Clean dashboards omit irrelevant data. Use the `when` builder (or a serializable JEXL expression) to conditionally hide entire sections or fields when they do not apply to the current record.

```ts
import { displaySection, when } from "@dyrected/sdk";
...
displaySection("Order & Fulfillment", [
  ...
  span: 12,
  icon: "Package",
  visible: when.any(when.fieldIsTrue("wantsAsoebi"), when.fieldIsTrue("wantsAsoOke")),
}),
```
