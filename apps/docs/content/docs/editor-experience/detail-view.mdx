...
## Open related rows in a drawer

When a Detail View shows related rows through a join field, clicking a row opens it in a side drawer instead of navigating away. The parent record stays on screen while the related document opens beside it, so editors never lose their place while fixing a line item.

The drawer follows the related collection's own Detail View. When that collection sets `detail` (a custom schema or `detail: true`), the drawer opens on the read-only summary with a pencil toggle to switch into the form. Collections without `detail` open straight into the form — the same edit-first behavior as the rest of the admin.

Inside the drawer, the Previous/Next buttons step through the sibling rows without closing it, and the Open-full-page button jumps to the record's full edit page when the change outgrows a quick fix. Saving in the drawer writes through the normal collection update pipeline, so hooks and access rules run exactly as they would on the edit page. On mobile the drawer becomes a bottom sheet.
...
displayField("coverImage", { display: "image", aspectRatio: "16/9", objectFit: "cover", align: "center", span: 12 });
```
...
- **Primary Status Pill**: (`display: "badge"`)
- **Secondary State**: Door check-in, fulfillment, stock level, or customer tier
- **Primary Channels**: Clickable phone (`display: "phone"`), email (`display: "email"`), or reference ID (`display: "copyable"`)
...
- Use **`displayGrid` + `displayDivider({ spacing: "md" })`** for the continuous primary narrative (Contact, Key Details, Action Hub).
- Reserve **`displaySection`** for complex, multi-field subsystems that benefit from distinct boundaries, status badges, or collapsible drawers (such as *Financial Breakdowns*, *Shipping Logistics*, or *Reviewer Notes*).
...
### 4. Enable micro-interactions with `editable: true`
...
Clean dashboards omit irrelevant data. Use the `expr` expression builder (or a serializable JEXL expression) to conditionally hide entire sections or fields when they do not apply to the current record.

```ts
import { displaySection, expr } from "@dyrected/sdk";
...
displaySection("Order & Fulfillment", [
  ...
  span: 12,
  icon: "Package",
  visible: expr.any(expr.isTrue("wantsAsoebi"), expr.isTrue("wantsAsoOke")),
}),
```
...
1. **Identity & Immediate Status**: Who is this and what is their current state?
2. **Primary Action Tools**: Instant pass preview, dispatch trigger, scanner.
3. **Operational Details**: Order quantities, payment breakdowns, fulfillment cards.
4. **Context & Personal Notes**: Well-wishes, message notes, events schedule.
5. **Timeline & Audit Metadata**: Submitted date, author ID, timestamps.
...
| Option | Description |
| --- | --- |
| <code>type</code> (required) | Identifies this item as an action button. |
| <code>name</code> (required) | Name of the action, matching an entry in &#96;collection.actions&#96;. |
| <code>options</code> (optional) | Display and visibility options for this placement. |
...
| <code>variant</code> (optional) | Button visual style. Defaults to &#96;'outline'&#96;. |
| <code>size</code> (optional) | Button size. Defaults to &#96;'sm'&#96;. |
| <code>label</code> (optional) | Overrides the action's own &#96;label&#96; for this placement. |
| <code>visible</code> (optional) | Visibility condition for this action item. Can be a boolean or a JEXL expression evaluated against &#96;&#123; doc, user &#125;&#96;. |
...
| <code>aspectRatio</code> (optional) | Specific aspect ratio constraint for media (e.g., "16/9", "4/3", "1/1", "auto"). |
| <code>objectFit</code> (optional) | CSS object-fit behavior for media ('cover', 'contain', 'fill', 'scale-down'). |
| <code>align</code> (optional) | Media alignment ('left', 'center', 'right'). |
| <code>width</code> (optional) | Fixed pixel width or CSS dimension for media preview. |
| <code>height</code> (optional) | Fixed pixel height or CSS dimension for media preview. |
