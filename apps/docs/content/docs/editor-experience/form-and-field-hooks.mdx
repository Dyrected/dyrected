...
## Declarative Field Transforms & `expr` Builder

For reactive field transforms in the Admin form (such as auto-generating slugs or sanitizing inputs), declare serializable `admin.hooks.onChange` expressions with the type-safe `expr` expression builder:

```typescript
import { defineTextField, expr } from "@dyrected/sdk";
...
The `expr` expression builder compiles to JEXL expressions evaluated with 30+ globally registered helper functions:

### String Helpers
- `slugify(str)`: Transforms text into a clean URL-safe slug (`slugify(siblingData.title)` $\to$ `"hello-world"`). Available via `expr.slugify(field)`.
- `lower(str)` / `upper(str)`: Converts string casing. Available via `expr.lower(field)` and `expr.upper(field)`.
- `trim(str)` / `capitalize(str)`: Removes surrounding whitespace or capitalizes the first word. Available via `expr.trim(field)`.
