...
Audit logging is different from runtime logging. Runtime logs help you monitor and debug the running service. Audit logs are durable content-history records stored in `__audit`. If you need request logs, redaction, tracing, or metrics, use [Logging and Observability](/docs/deployment-and-operations/infrastructure/logging).
