...
The columns an editor sees come from `admin.defaultColumns` on the collection, falling back to the first few fields. Editors can then reorder columns or toggle them on and off with the **View** button — and those choices are remembered per person. Because this is personal (with a team-wide default admins can publish), it has its own page: see [Preferences](/docs/editor-experience/preferences) for the full cascade and how to set defaults.
...
- **[Spreadsheet View](/docs/editor-experience/spreadsheet-view)** — switch the list into a grid and edit many records inline, like a spreadsheet, without opening each edit form.
