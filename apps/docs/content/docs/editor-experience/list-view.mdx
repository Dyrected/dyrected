...
The columns an editor sees come from `admin.defaultColumns` on the collection, falling back to the first few fields — or from the view's `columns` when you're in an operational view (`defineView({ columns: [...] })`). Editors can then reorder columns or toggle them on and off with the **View** button — and those choices are remembered per person via `layout:collections:${slug}:view:${viewSlug}` (see [Routing and preferences](/docs/model-content/operational-views/routing-and-preferences) for legacy `view-columns:` fallback). Because this is personal (with a team-wide default admins can publish), it has its own page: see [Preferences](/docs/editor-experience/preferences) for the full cascade and how to set defaults.

The table uses a single unified scroll container with a sticky header row and sticky selection column. As editors scroll long lists or wide schemas horizontally and vertically on desktop and mobile, column headers and selection controls stay pinned in view.
...
The same `admin` block shapes the default list before editors touch it: `filter` narrows which records the list opens on, `sort` sets the opening order, `groupBy` organizes it into collapsible sections, and `metrics` adds KPI cards above the table. These mirror the same keys on `defineView`, so moving a collection to curated views later keeps the behavior editors already know.
...
Both respect access control. Delete is disabled for a record the current editor can't delete, and — on an auth collection — editors can't delete their own account. If a whole action is missing for someone, that's their [access rules](/docs/model-content/content-rules/access-control/overview) at work, not a bug.
...
- **Export Selected** — download just the selected rows as a CSV, using the same format as a full [CSV export](/docs/editor-experience/csv-import-export). This is the quick way to pull an ad-hoc subset without exporting the whole collection.
...
- **[Spreadsheet View](/docs/editor-experience/spreadsheet-view)** — inline grid for the same records; now also a first-class `layout: "spreadsheet"` view (`/docs/model-content/operational-views/layouts/spreadsheet`) rather than only a toggle on the legacy list.
- **[CSV Import & Export](/docs/editor-experience/csv-import-export)** — bulk-import records from a CSV, and export the whole collection to a CSV file.

## Recommended path

Set `admin.useAsTitle`, `admin.defaultColumns`, and `admin.searchableFields` so the list is useful the moment a collection exists. Add `admin.description` when editors would benefit from a one-line explanation under the page title. Let editors tailor their own view with the **View** button, use single-row Edit/Delete for everyday changes, and reach for bulk selection when clearing several records. When the work is editing rather than deleting, move to [Spreadsheet View](/docs/editor-experience/spreadsheet-view) or [CSV Import & Export](/docs/editor-experience/csv-import-export).
