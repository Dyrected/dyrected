...
- **Layout-specific workspaces** — `table` (tablecn + TanStack), `kanban` (@dnd-kit board), `calendar` (ReUI event calendar), `cards` (gallery), `spreadsheet` (inline grid). Each respects `columns`, `filter`, `sort`, and persists column/view choices per editor.
