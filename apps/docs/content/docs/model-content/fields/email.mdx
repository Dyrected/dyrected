...
<Note>
Authentication collections already include an `email` field that Dyrected injects for you. You can define your own instead: Dyrected keeps your definition but always re-asserts the parts the login flow depends on — `required`, `unique`, and its [access `update` rule](/docs/model-content/content-rules/access-control/fields) — so a custom `email` field can never accidentally allow duplicate accounts.
</Note>
