...
Like the text length limits, `min` and `max` guide the editing experience but do not reject an out-of-range value on their own. When you need a hard rule — a strict range, rounding, or a computed total — add a [hook](/docs/model-content/content-rules/hooks) that runs on write, so it applies on every path and not just the admin form.

## Filtering and sorting

If you filter or sort on a number often, mark it `promoted` so it becomes an indexed column. See [Database indexes](/docs/deployment-and-operations/infrastructure/database/indexes).
...
`admin.format` only changes presentation. If you need the *stored* value to be rounded, clamped, or derived — a total that must always equal quantity × price, say — that's a job for a [hook](/docs/model-content/content-rules/hooks) that runs on write, so the rule holds on every path and not just the admin form.
