...
Sometimes a field only makes sense in one layout — an avatar photo on a testimonial card, but not on a plain pull quote. Use the `when` builder to declare clean variant visibility in your field's `admin.condition`:

```ts
import { defineBlock, defineImageField, defineTextField, when } from "@dyrected/sdk";
...
defineBlock({
  ...
  fields: [
    ...
    defineImageField({
      ...
      label: "Avatar",
      relationTo: "media",
      admin: { condition: when.variant("card") },
    }),
  ],
});
```

Now the avatar picker appears only while the card variant is selected. The `when.variant("card")` condition generates a serializable expression evaluated against the block row's own values, so `variant` always reflects the row's current selection — blocks added in the Admin start with the first variant already set. You can also match multiple variants with `when.variant("card", "split")` or everything except a variant with `when.notVariant("quote")`.
