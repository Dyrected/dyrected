...
Each row stores the chosen block's `slug` as `blockType`, so your frontend can map each `blockType` to a component and render the list in order — the [`Blocks` component](/docs/deliver-content/displaying-content/overview) does that mapping for you. Keep block `slug` values stable once content exists — renaming a slug orphans the rows already saved under the old name — and have your renderer fall back gracefully when it meets a `blockType` it does not recognize.
...
Sometimes a field only makes sense in one layout — an avatar photo on a testimonial card, but not on a plain pull quote. Use the `expr` expression builder to declare clean variant visibility in your field's `admin.condition`:

```ts
import { defineBlock, defineImageField, defineTextField, expr } from "@dyrected/sdk";
...
defineBlock({
  ...
  fields: [
    ...
    defineImageField({
      ...
      label: "Avatar",
      relationTo: "media",
      admin: { condition: expr.variant("card") },
    }),
  ],
});
```

Now the avatar picker appears only while the card variant is selected. The `expr.variant("card")` condition generates a serializable expression evaluated against the block row's own values, so `variant` always reflects the row's current selection — blocks added in the Admin start with the first variant already set. You can also match multiple variants with `expr.variant("card", "split")` or everything except a variant with `expr.notVariant("quote")`.
