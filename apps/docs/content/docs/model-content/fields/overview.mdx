...
- `immutable` — the value can be set when the document is created but never changed afterwards. An update that tries to change it fails with a `400`; re-sending the same value is fine, so full-document saves from the admin keep working. Good for currencies, external references, and other values a ledger relies on.
