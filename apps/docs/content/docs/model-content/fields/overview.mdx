...
- `immutable` — the value can be set when the document is created but never changed afterwards. An update that tries to change it fails with a `400`; re-sending the same value is fine, so full-document saves from the admin keep working. Good for currencies, external references, and other values a ledger relies on. It applies to top-level fields on collections (including ones inside a `row`), not to fields nested inside `object`, `array` or `blocks`, and not to globals.
...
- Numbers and dates: [Number](/docs/model-content/fields/number), [Money](/docs/model-content/fields/money), [Date](/docs/model-content/fields/date), [Datetime](/docs/model-content/fields/datetime), [Time](/docs/model-content/fields/time)
...
| <code>immutable</code> (optional) | Whether the value can only be set when the document is created. Any update that tries to change it is rejected with a validation error. Re-sending the unchanged value is allowed. |
| <code>defaultValue</code> (optional) | Default value: literal value or function computed from context. |
...
| <code>ai</code> (optional) | AI assistant privacy and PII redaction settings for this field. |
