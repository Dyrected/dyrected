...
Read the [Access Control overview](/docs/model-content/content-rules/access-control/overview) first for the shared model — the four rule shapes and the context a rule receives. This page covers the two rules a global supports.

<CloudOnly>

For Cloud globals, use booleans, Jexl strings, or serializable named policies. That keeps site settings, navigation, and other singleton content enforceable after schema sync.
...
Global rules run on the server with the standard [access context](/docs/model-content/content-rules/access-control/overview#what-a-rule-can-see). The useful values here are:
...
For Cloud, keep that logic serializable. This example lets anyone read a public global, and lets admins update it only while it is not locked:

```ts
access: {
  read: "doc.visibility == 'public'",
  update: "'admin' in user.roles && doc.locked != true",
}
```
...
- [Global configuration](/docs/model-content/configuration/globals) — everything else you can set on a global.
- [Global hooks](/docs/deployment-and-operations/server-runtime/hooks/globals) — for logic that transforms data or raises custom errors, beyond allow/deny.
