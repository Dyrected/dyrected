...
This page is the model: what a rule is, the shapes it can take, when it runs, and what it can see. The pages that follow show where to put rules for [collections](/docs/model-content/content-rules/access-control/collections), [globals](/docs/model-content/content-rules/access-control/globals), and [fields](/docs/model-content/content-rules/access-control/fields). If you are deploying to Dyrected Cloud, also read [Dyrected Cloud access control](/docs/model-content/content-rules/access-control/cloud) for the Cloud-specific policy limits and built-in policy names.
...
- An **object** result allows the operation but scopes it to matching documents — a row-level filter, covered in [Collections](/docs/model-content/content-rules/access-control/collections#document-aware-rules).
...
| **Named policy** | You want a reusable rule you can name once and reference everywhere. Serializable policies work with Cloud; function policies stay self-hosted. | Server |
...
A rule that evaluates to an **object** becomes a row-level `where` filter — `"{ owner: { equals: user.sub } }"` limits the operation to the caller's own documents (covered in [Collections](/docs/model-content/content-rules/access-control/collections#document-aware-rules)). A ternary lets one rule return either shape, so it can hand signed-in users everything and scope everyone else:
...
If you need a policy with parameters, use one of the Cloud built-ins. For the current built-in policy list and common parameters, see [Dyrected Cloud access control](/docs/model-content/content-rules/access-control/cloud#built-in-named-policies-in-dyrected-cloud).
...
<Tabs items={['Jexl string', 'Named policy']}>
...
A policy can be a **function** or a **Jexl string** — `accessPolicies: { isAdmin: "'admin' in user.roles" }`. String policies are inlined when the schema is sent to the admin panel, so a field or collection rule that references one stays reactive in the edit form, exactly like an inline Jexl rule. Function policies run on the server only and are not Cloud-portable.
...
| Value | What it is |
|---|---|
| `user` | The authenticated caller, or `undefined` for an anonymous request |
| `req` | The request context |
| `doc` | The existing document, when Dyrected is checking one |
| `data` | The incoming payload, on create and update |
| `params` | Extra values passed by `{ policy: 'name', params: { ... } }` |
...
The scaffolded auth collection ships with a `roles` field offering `admin`, `editor`, and `viewer`, which is why `'admin' in user.roles` is the idiom you will see most often. For the full picture of what lands on `user`, see [Token data](/docs/deployment-and-operations/authentication/token-data).
...
|---|---|---|
| [Collections](/docs/model-content/content-rules/access-control/collections) | `read`, `create`, `update`, `delete`, `readAudit` | Whole documents in a collection, plus the audit log |
| [Globals](/docs/model-content/content-rules/access-control/globals) | `read`, `update` | A single global document |
| [Fields](/docs/model-content/content-rules/access-control/fields) | `read`, `create`, `update` | Individual fields — stripped from responses or dropped on writes |
...
- [Collection access control](/docs/model-content/content-rules/access-control/collections) — the four operations, row-level filtering, and common recipes.
- [Global access control](/docs/model-content/content-rules/access-control/globals) — read and update rules for single-document config.
- [Field access control](/docs/model-content/content-rules/access-control/fields) — hiding and locking individual fields.
- [Hooks](/docs/model-content/content-rules/hooks) — for logic that needs to transform data or raise custom errors, not just allow or deny.
