...
- `tasks` for recurring [background tasks](/docs/deployment-and-operations/server-runtime/tasks)
...
See [Collections](/docs/model-content/configuration/collections).

### `globals`

Globals define singleton documents such as navigation, SEO defaults, or theme settings.

See [Globals](/docs/model-content/configuration/globals).
...
See [Logging and Observability](/docs/deployment-and-operations/infrastructure/logging).
...
- Read [Collections](/docs/model-content/configuration/collections) when the content has many entries.
- Read [Globals](/docs/model-content/configuration/globals) when the content should exist once.
- Read [Environment Variables](/docs/deployment-and-operations/infrastructure/environment-variables) before wiring secrets and framework-specific URLs.
- Go back to [Concepts](/docs/model-content/concepts) if you want the high-level mental model first.
...
| Option | Description |
| --- | --- |
| <code>navigation</code> (optional) | Custom navigation layout for the admin sidebar. If omitted, all collections and globals appear in default sections. |
...
| <code>adminUrl</code> (optional) | Public URL of the Admin UI, used for links in invite and password-reset emails. Falls back to the &#96;DYRECTED_ADMIN_URL&#96; env var, then the page the admin is running on, then &#96;/admin&#96; (resolved against the request origin). |
...
| Option | Description |
| --- | --- |
| <code>collection</code> (required) | Collection slug. |
| <code>aggregates</code> (required) | Named aggregate operations to compute. |
| <code>groupBy</code> (optional) | Optional field to group the aggregates by. |
...
| <code>media</code> (optional) | Media management, dynamic transformations, and transformation preset configurations. |
...
| <code>tasks</code> (optional) | Background tasks. Define each with &#96;defineTask&#96;, then run them with &#96;createTaskRunner(config)&#96;. |
...
| <code>ai</code> (optional) | AI Assistant and LLM tools configuration. |
| <code>trash</code> (optional) | App-wide trash &amp; retention configuration. |
