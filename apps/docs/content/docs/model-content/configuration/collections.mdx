...
- the `when` condition builder (or serializable JEXL string), which is the recommended default
- a custom function that returns the URL from document data (self-hosted only)

Prefer the `when` builder (`when.concat`, `when.then`, or `when.match`). It keeps your configuration 100% serializable, provides TypeScript autocomplete, and runs safely across Dyrected Cloud and headless runtimes.
...
For single-prefix collections, use `when.concat`:
...
export const Posts = defineCollection({
  ...
  admin: {
    useAsTitle: "title",
    // Generates: '/blog/' + slug
    previewUrl: when.concat("/blog/", "slug"),
    urlPattern: "/blog/{slug}",
  },
  fields: [
    defineTextField({ name: "title", label: "Title", required: true }),
...
For conditional fallback routing (such as routing a `home` slug directly to `/`), use `when.then`:

```ts
// Routes 'home' to '/', and all other pages to '/slug'
previewUrl: when.then(when("slug").equals("home"), "/", when.concat("/", "slug")),
```

For multi-branch category routing, use `when.match`:

```ts
previewUrl: when.match()
  .case(when("slug").equals("home"), "/")
  .case(when("category").equals("news"), when.concat("/news/", "slug"))
  .case(when("category").equals("portal"), when.concat("/portals/", "slug"))
