...
If you omit `views`, Dyrected synthesizes a `list` table view from `admin.defaultColumns` or the first five display fields — so `/collections/:slug` always has a route. Keep `slug` and view `slug`s stable; they are part of the URL and preference keys.

You can shape that default table without a `views` array. These `admin` options apply directly to the default list and match the same keys on `defineView`:

```ts
export const Orders = defineCollection({
  slug: "orders",
  admin: {
    defaultColumns: ["customer", "total", "status"],
...
`filter` narrows the default list before the editor's toolbar filters, `sort` sets the opening order, `groupBy` organizes it into collapsible sections, and `metrics` adds KPI cards on top. When a default view redirect (`defaultView`, or a view with `default: true`) is configured, that view's own settings win on its route.

For layouts, actions, metrics, and UX taste, read [Operational Views](/docs/model-content/operational-views/overview).
...
For the full field system, read [Fields](/docs/model-content/fields/overview).
...
Use a custom Detail View when the record is easier to understand as a summary than as a full form. For the full guide, read [Detail Views](/docs/editor-experience/detail-view).
...
For more on page sections, read [Blocks](/docs/model-content/fields/blocks).
...
| <code>trash</code> (optional) | Trash &amp; retention settings for this collection. If &#96;true&#96; or an object, deleting a document moves it to trash rather than destroying it. If &#96;false&#96;, hard deletes immediately. |
| <code>access</code> (optional) | Collection-level access control.  Each key is an operation; the value can be a function, a Jexl string, a boolean, or a named policy reference. Returning &#96;true&#96; allows access and &#96;false&#96; denies it. Returning a &#96;where&#96;-style object grants access only to matching documents.  See: [Access control overview](https://docs.dyrected.com/docs/self-hosted/model-content/content-rules/access-control/overview) |
| <code>hooks</code> (optional) | Collection-level lifecycle hooks.  Hooks run in the order they appear in the array. The return value of each hook is passed as the input to the next. Throwing inside any hook aborts the operation and returns a &#96;500&#96; error.  See the Hooks reference for the full lifecycle diagram.  See: [Hooks overview](https://docs.dyrected.com/docs/self-hosted/model-content/content-rules/hooks), [Collection hooks](https://docs.dyrected.com/docs/self-hosted/deployment-and-operations/server-runtime/hooks/collections) |
| <code>admin</code> (optional) | Admin UI configuration for this collection.  See: [Admin options](https://dyrected.com/docs/model-content/configuration/collections#admin-options) |
| <code>ai</code> (optional) | Optional custom AI assistant instructions and RAG indexing options for this collection. |
| <code>detail</code> (optional) | Detail view layout configuration for the Admin UI.  By default, collections are **Edit-First** (&#96;detail: false&#96;). Clicking a record in the table or cards view navigates directly to the editing form with Live Preview.  Set to &#96;true&#96; to enable the automatic read-only summary layout, or provide a &#96;DetailSchema&#96; array using display helpers (&#96;displaySection&#96;, &#96;displayField&#96;, &#96;displayTabs&#96;, etc.) for a custom operational dashboard layout. |
| <code>defaultView</code> (optional) | Slug of the operational view to render by default when navigating to &#96;/collections/:slug&#96;.  When set: 1. Navigating to &#96;/collections/:slug&#96; automatically redirects to &#96;/collections/:slug/views/:defaultView&#96;. 2. The main collection link in the sidebar directly opens this default view. 3. The sidebar submenu cleanly lists only your defined views without showing a generic &#96;All [Collection]&#96; item. |
| <code>views</code> (optional) | Tailored operational workspaces for this collection (&#96;table&#96;, &#96;spreadsheet&#96;, &#96;kanban&#96;, &#96;calendar&#96;, &#96;cards&#96;, &#96;gantt&#96;). Each view provides curated columns, filters, metrics, and workflow buttons for a specific job.  See: [Operational views overview](https://dyrected.com/docs/model-content/operational-views/overview) |
| <code>actions</code> (optional) | Custom workflow actions for this collection, independent of any single view. These run against one document at a time (no &#96;viewSlug&#96; required), so they work anywhere a document is in scope — most notably the read-only Detail View, where they can render in the header toolbar (&#96;type: 'header'&#96;) and/or inline via &#96;displayAction(name)&#96; in the &#96;detail&#96; schema. The same action definitions can also be referenced from a &#96;defineView&#96;'s own &#96;actions&#96; for list-page use — they are not mutually exclusive. |
...
See [Access Control Overview](/docs/model-content/content-rules/access-control/overview) for the full model.
...
See [Hooks Overview](/docs/model-content/content-rules/hooks) for the lifecycle and [Collection Hooks](/docs/deployment-and-operations/server-runtime/hooks/collections) for collection-focused examples.
...
If your organization uses custom role naming (such as `super_admin` instead of the default `admin`), pass `adminRole` in your auth config:
...
export const Admins = defineCollection({
  slug: "admins",
  auth: {
    adminRole: "super_admin", // Defaults to 'admin'
    maxLoginAttempts: 5,
    lockTime: 15 * 60 * 1000,
  },
  fields: [
...
When configured, Dyrected grants this role to the initial user created through `/api/collections/:slug/first-user` and checks for it when verifying administrative privileges in the Admin UI.

### Custom Action URLs and Expirations

For collections representing external end users (such as `customers` or `subscribers`), you typically want invite and reset links to direct users to your custom frontend app rather than the Dyrected Admin dashboard.

Configure `urls` and token expiration settings directly in `auth`:
...
export const Customers = defineCollection({
  ...
  auth: {
    ...
    inviteExpiration: "14d", // Defaults to '7d'
    resetPasswordExpiration: "30m", // Defaults to '1h'
    email: {
      templates: {
        invite: ({ url }) => ({
...
- `urls.invite`: base URL used to construct invite acceptance links (`${url}?token=<token>`).
- `urls.resetPassword`: base URL used to construct password reset links (`${url}?token=<token>`).
- `inviteExpiration`: lifetime of the invitation token (e.g. `'7d'`, `'24h'`, or milliseconds).
- `resetPasswordExpiration`: lifetime of the reset token (e.g. `'1h'`, `'30m'`, or milliseconds).
- `email.templates`: collection-specific template overrides that take precedence over global templates.

For the full auth model, token behavior, and login strategies, read [Authentication Overview](/docs/editor-experience/editor-accounts).
...
For storage behavior and adapter-level upload setup, read [Upload Overview](/docs/model-content/media/overview). For the field-level side of file relationships, read [Upload Field](/docs/model-content/fields/image).
...
This is a larger feature with its own states, transitions, and roles, so it lives on its own page. For the full model, read [Workflows Overview](/docs/editor-experience/publishing/overview).
...
For what gets recorded and how it works, read [Audit Overview](/docs/editor-experience/audit-history).
...
`admin.previewUrl` can be configured using:

- the `expr` expression builder (or serializable JEXL string), which is the recommended default
- a custom function that returns the URL from document data (self-hosted only)

Prefer the `expr` builder (`expr.concat`, `expr.ifElse`, or `expr.match`). It keeps your configuration 100% serializable, provides TypeScript autocomplete, and runs safely across Dyrected Cloud and headless runtimes.

`admin.urlPattern` is different: it is a plain route pattern string that uses `{fieldName}` placeholders, such as `/blog/{slug}` or `/{locale}/{slug}`.

### Dynamic Preview Routing Examples

For single-prefix collections, use `expr.concat`:
...
export const Posts = defineCollection({
  ...
  admin: {
    useAsTitle: "title",
    // Generates: '/blog/' + slug
    previewUrl: expr.concat("/blog/", "slug"),
    urlPattern: "/blog/{slug}",
  },
  fields: [
    defineTextField({ name: "title", label: "Title", required: true }),
...
For conditional fallback routing (such as routing a `home` slug directly to `/`), use `expr.ifElse`:

```ts
// Routes 'home' to '/', and all other pages to '/slug'
previewUrl: expr.ifElse(expr("slug").equals("home"), "/", expr.concat("/", "slug")),
```

For multi-branch category routing, use `expr.match`:

```ts
previewUrl: expr.match()
  .case(expr("slug").equals("home"), "/")
  .case(expr("category").equals("news"), expr.concat("/news/", "slug"))
  .case(expr("category").equals("portal"), expr.concat("/portals/", "slug"))
...
For field-level modeling, read [Fields](/docs/model-content/fields/overview).

Hooks are optional here. You only need them if you want Dyrected to derive routable values automatically, such as generating a document slug from a title before save. For that workflow, read [Hooks Overview](/docs/model-content/content-rules/hooks).
...
- See [Globals](/docs/model-content/configuration/globals) for singleton content.
- See [Overview](/docs/model-content/configuration/overview) for the full root config shape.
- See [Fields](/docs/model-content/fields/overview) for the schema system that powers collection documents.
