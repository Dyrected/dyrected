...
- **Never use function callbacks in `admin.condition`** if the project uses Dyrected Cloud — use Jexl string expressions (Jexl is a small, safe expression language that travels as a string); functions are stripped on cloud sync
