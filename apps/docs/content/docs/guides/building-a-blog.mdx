...
<Tab value="Nuxt.js">
Adding the `@dyrected/nuxt` module auto-imports the `<DyrectedAdmin>` component, but it does not create a route for you — you decide where the dashboard lives. Create a page and render `<DyrectedAdmin>` with a `basename` that matches that route. Here it is at `/admin`; use a different filename and `basename` (say `pages/cms-admin.vue` with `basename="/cms-admin"`) if you want a different path.
