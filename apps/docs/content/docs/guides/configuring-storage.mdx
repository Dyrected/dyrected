...
```bash
curl -X POST https://yoursite.com/dyrected/collections/media \
...
**When to implement `resolve()`:** Only if your adapter stores files somewhere that cannot be accessed with a direct public URL — for example, on the local filesystem in a self-hosted setup. Dyrected calls `resolve()` to serve the file through its own API at `/dyrected/media/<filename>`.
