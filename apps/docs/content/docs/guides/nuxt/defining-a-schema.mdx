...
Use this page after [Installing Dyrected](/docs/guides/nuxt/installing-dyrected). At this point, Dyrected is already mounted into your app and `/admin` should load. The next job is to turn the starter config into a schema that matches your real site instead of the generic example content.
...
- which document shapes your Nuxt code will receive

So the first schema question is not "what might be editable one day?" It is "what does this site already need right now?"

## Keep the first version small

For most content-managed Nuxt sites, the best first schema is still small:
...
For a typical Nuxt site, the `pages` collection is usually the first content type worth getting right.
...
## Splitting your schema into multiple files

As your application grows, keeping all collections and globals inside a single `dyrected.config.ts` can become crowded. You can split collections and globals into their own TypeScript files and import them cleanly:
...
Then import your definitions into `dyrected.config.ts`:

```ts
import { defineConfig } from "@dyrected/core";
import { Pages } from "./dyrected/collections/pages";
import { Posts } from "./dyrected/collections/posts";
...
Because the Nuxt module loads your configuration using on-the-fly TypeScript resolution, you can use standard extensionless relative imports without worrying about ESM loader mismatches.
...
- [Concepts](/docs/model-content/concepts)
...
Once that is true, continue to [Setting Up Initial Data](/docs/guides/nuxt/setting-up-initial-data).
