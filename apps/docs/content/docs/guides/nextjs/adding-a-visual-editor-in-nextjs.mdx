...
For block content, that means rendering through `Blocks` and using `useDyPath()` inside the block component:
...
export function HeroBlock({ heading, subheading }: { heading: string; subheading?: string }) {
  // Call useDyPath() once at the top of your component
  const dy = useDyPath();
...
Continue to [Let Editors Build Custom Pages in Next.js with Dyrected](/docs/guides/frameworks/nextjs/building-landing-pages-for-marketing).
...
Once that is true, the preview integration is working. Continue to [Let Editors Build Custom Pages in Next.js with Dyrected](/docs/guides/frameworks/nextjs/building-landing-pages-for-marketing).
