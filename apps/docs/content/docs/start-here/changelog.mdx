...
- View Header Action Execution:

  - Allow `type: 'header'` actions to execute without requiring document IDs in `@dyrected/admin` and `@dyrected/core`.
  - Pass `{ doc: null, docs: [], user, input, collection }` to header action handlers, returning handler results or defaulting to `{ success: true }`.
  - Prevent dropped clicks in the admin view toolbar when `ids.length === 0`. (`@dyrected/admin`, `@dyrected/core`)
