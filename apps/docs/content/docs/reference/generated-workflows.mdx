...
Use `publishingWorkflow()` for the standard draft → review → publish lifecycle, or `definePublishingWorkflow()` to map your own role names onto its editor and publisher tiers. For a lighter draft/published toggle, `simplePublishingWorkflow()` is what the `drafts: true` collection shortcut uses under the hood. Define a fully custom workflow only when the business process genuinely has different states, capabilities, or transition rules.
