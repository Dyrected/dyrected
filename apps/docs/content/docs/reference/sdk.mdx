...
| `greater_than` | Numeric/Date comparison | `{ price: { greater_than: 100 } }` |
| `less_than` | Numeric/Date comparison | `{ price: { less_than: 500 } }` |
| `like` | Case-insensitive contains | `{ title: { like: 'typescript' } }` |

**Combining Filters:**
You can use `and` and `or` to group filters:
