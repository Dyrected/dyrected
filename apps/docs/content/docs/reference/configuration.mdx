...
| `admin` | `object` | UI customizations like `icon`, `useAsTitle`, `defaultColumns`, and `urlPattern`. |
| `workflow`| `WorkflowConfig`| Optional. Enables editorial approval states and capability gating. |

### Collection `admin` options

| Property | Type | Description |
| :--- | :--- | :--- |
| `icon` | `AdminIconName` | Valid Lucide component name for the sidebar, such as `Newspaper`. |
| `useAsTitle` | `string` | Field name to use as the document title in the Admin UI list view. |
| `defaultColumns` | `string[]` | Columns shown by default in the list view. |
| `group` | `string` | Groups this collection under a named sidebar heading. |
| `hidden` | `boolean` | Hides the collection from the Admin UI sidebar entirely. |
| `previewUrl` | `string \| function` | URL or jexl expression for the Live Preview pane. See [Live Preview](/docs/guides/live-preview). |
| `urlPattern` | `string` | Frontend URL pattern for this collection, used by `url` fields to resolve internal links. Use `{fieldName}` placeholders — e.g. `"/{slug}"` or `"/blog/{slug}"`. See [URL field](/docs/reference/fields#url). |
...
| `admin.icon` | `AdminIconName` | Valid Lucide component name for the global's sidebar icon. |
...
See [Email](/docs/guides/sending-email) for the full guide — provider examples, the Ethereal dev fallback, and custom template overrides.
