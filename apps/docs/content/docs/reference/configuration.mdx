...
| `admin` | `object` | UI customizations like `useAsTitle`, `defaultColumns`, and `urlPattern`. |
...
| `previewUrl` | `string \| function` | URL or jexl expression for the Live Preview pane. See [Live Preview](/docs/features/live-preview). |
| `urlPattern` | `string` | Frontend URL pattern for this collection, used by `url` fields to resolve internal links. Use `{fieldName}` placeholders — e.g. `"/{slug}"` or `"/blog/{slug}"`. See [URL field](/docs/concepts/fields#url). |
