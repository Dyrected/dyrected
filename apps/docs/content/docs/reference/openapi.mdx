...
When a collection has `workflow` enabled, the OpenAPI spec automatically includes definitions for `POST /api/collections/:slug/:id/transitions/:transition` and `GET /api/collections/:slug/:id/workflow-history` along with their exact request/response schemas.
