...
### Transition a document workflow
...
Atomic state transition for collections with `workflow` enabled. Can require comment or reject/publish capabilities depending on the workflow config. 
...
**Response:** The document with its newly updated `_workflow` state.
...
Returns the history of all transitions applied to this document.

**Example:**
```bash
GET /api/collections/posts/abc123/workflow-history?limit=10
```

**Response:** Standard paginated list of `WorkflowHistoryEntry` records.
