...
> **On Dyrected Cloud, storage is fully managed.** No adapter config needed — files are stored in Dyrected's cloud storage with automatic CDN delivery. [Learn more →](/docs/deployment/cloud)
...
| `CloudinaryStorageAdapter` | `https://res.cloudinary.com/cloud-name/image/upload/image.jpg` |
...
Configure the upload behaviour per collection (see also [Upload Collections](/docs/guides/file-uploads)):
