...
If you are targeting Cloud, read [Cloud-safe hooks](/docs/model-content/content-rules/cloud-safe-hooks) alongside this page. That page is the canonical guide for what survives `sync:schema` and what gets stripped.
...
For Cloud-safe `beforeRead` details, including the exact expression context, see [Cloud-safe hooks](/docs/model-content/content-rules/cloud-safe-hooks).

<Note>
Reach for [access control](/docs/model-content/content-rules/access-control/collections) first when the question is who is allowed to see a document at all. Reach for `beforeRead` when the question is how an already-allowed read should be shaped or narrowed at the hook layer.
</Note>
...
For Cloud-safe `afterRead` details, including merge behavior for object returns, see [Cloud-safe hooks](/docs/model-content/content-rules/cloud-safe-hooks).
...
For Cloud-safe `beforeChange` details, including the exact expression context and merge behavior, see [Cloud-safe hooks](/docs/model-content/content-rules/cloud-safe-hooks).
...
`beforeCommit` runs inside the database transaction that saves a create or update, after the document is written and before the transaction commits. It receives the saved `doc`, the `previousDoc` on updates, the operation, and `tx`, a writable adapter tied to that transaction.

Use it when a second write has to stand or fall with the first, such as a ledger row for a payment:
...
hooks: {
  beforeCommit: [
    async ({ doc, operation, tx }) => {
      ...
      await tx.create({
        collection: 'ledger',
        data: { paymentId: doc.id, amountMinor: doc.amountMinor },
      })
    },
  ],
}
...
If the hook throws, the transaction rolls back, so neither the document nor the ledger row is saved and the request fails. That is the difference from `afterChange`, which runs after the commit and can never undo the write.

It runs for creates and updates made through the API, the admin, and [operational view actions](/docs/model-content/operational-views/actions), including on collections with a workflow. It does not run for file uploads, and globals have no `beforeCommit` phase.

Keep the work in here to database reads and writes, because the transaction stays open while the hook runs (see [Transactions](/docs/deployment-and-operations/infrastructure/database/transactions)). It needs a transactional database adapter, and `beforeCommit` is function-only, so it is not part of the Cloud-safe declarative subset.
...
- Prefer declarative string hooks on `beforeRead`, `afterRead`, and `beforeChange` when you want the same behavior in Cloud.
- Use `previousDoc` in `afterChange` when the side effect depends on what changed.
- Keep access decisions in [access control](/docs/model-content/content-rules/access-control/collections) when the question is allow or deny. Use hooks when the question is transform, validate, react, or reshape an already-authorized operation.

## Related pages

- [Hooks overview](/docs/model-content/content-rules/hooks) for the full map of hook families.
- [Cloud-safe hooks](/docs/model-content/content-rules/cloud-safe-hooks) for the Cloud-specific subset and sync behavior.
- [Global hooks](/docs/deployment-and-operations/server-runtime/hooks/globals) for the singleton lifecycle.
- [Field hooks](/docs/deployment-and-operations/server-runtime/hooks/fields) for value-level transforms.
- [Hook context](/docs/deployment-and-operations/server-runtime/hooks/context) for the `req`, `user`, and `db` arguments these hooks receive.
