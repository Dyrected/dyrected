...
- [Collection hooks](/docs/deployment-and-operations/server-runtime/hooks/collections) for the full document lifecycle on collections.
- [Cloud-safe hooks](/docs/model-content/content-rules/cloud-safe-hooks) for the Cloud-specific compatibility rules.
- [Lifecycle events](/docs/editor-experience/publishing/lifecycle-events) for content events, webhooks, and self-hosted handlers.
- [Global hooks](/docs/deployment-and-operations/server-runtime/hooks/globals) for singleton settings and site-wide config.
- [Field hooks](/docs/deployment-and-operations/server-runtime/hooks/fields) for value-level transforms and admin form reactivity.
- [Hook context](/docs/deployment-and-operations/server-runtime/hooks/context) for the `req`, `user`, and `db` values hooks receive.
...
| <code>doc</code> (optional) | The targeted record / document when available. |
| <code>docs</code> (optional) | Array of targeted records for bulk action operations. |
| <code>user</code> (optional) | Current authenticated user. |
| <code>setValue</code> (optional) | Imperative setter for current field or sibling fields. |
...
| Option | Description |
| --- | --- |
| <code>value</code> (optional) | Current field value in the form state (if set). |
| <code>siblingData</code> (required) | Current values for sibling fields at the same nesting level. |
| <code>data</code> (required) | Current values for the entire form. |
| <code>doc</code> (optional) | The targeted record / document when available. |
| <code>docs</code> (optional) | Array of targeted records for bulk action operations. |
