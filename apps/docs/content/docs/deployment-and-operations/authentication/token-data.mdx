...
For how the token is signed and verified, see [JWT Strategy](/docs/deployment-and-operations/authentication/jwt-strategy).
...
- **A separate service that only has the token sees only identity.** If another system verifies a Dyrected token directly, it gets `sub`, `email`, and `collection` — to act on roles it must look the user up by `sub`. See [JWT Strategy](/docs/deployment-and-operations/authentication/jwt-strategy#verifying-a-token-in-another-service).

## Where to go next

- [JWT Strategy](/docs/deployment-and-operations/authentication/jwt-strategy) — signing, expiry, and external verification
- [Access Control](/docs/model-content/content-rules/access-control/overview) — turn the `user` object into permissions
- [Operations](/docs/deployment-and-operations/authentication/operations) — the endpoints that issue these tokens
