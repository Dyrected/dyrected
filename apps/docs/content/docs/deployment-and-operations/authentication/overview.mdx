...
`maxLoginAttempts` controls how many bad passwords Dyrected allows before locking the account. `lockTime` controls how long that lock lasts, in milliseconds. `tokenExpiration` controls how long session tokens stay valid — it accepts values like `'15m'`, `'12h'`, or `'30d'`, and defaults to `'7d'` when you leave it out.
...
Tokens still expire on their own after 7 days by default (set `auth.tokenExpiration` to change that), but there is a real server-side invalidation point too.
