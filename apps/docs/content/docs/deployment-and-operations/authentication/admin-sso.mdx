...
If you only need local email-and-password login for a handful of editors, you don't need any of this — see [Handing off to editors](/docs/editor-experience/hand-off-to-editors) instead.
...
<Callout type="warn">
`__admins` **always wins**. If your config contains an `__admins` collection, Dyrected uses it as the admin auth collection even when `collectionSlug` points somewhere else — the explicit slug only applies when no `__admins` exists (and, failing that, Dyrected falls back to the first collection with `auth: true`). If your login succeeds but `/me` fails, this precedence is the usual cause. See [Handing off to editors](/docs/editor-experience/hand-off-to-editors) for how the `__admins` gateway keeps customer and admin logins apart.
</Callout>
</Step>
...
- [Authentication overview](/docs/editor-experience/editor-accounts) — the collection-auth model this sits alongside
- [Handing off to editors](/docs/editor-experience/hand-off-to-editors) — onboarding editors and keeping customer login separate from admin
- [JWT Strategy](/docs/deployment-and-operations/authentication/jwt-strategy) — how Dyrected signs the admin token it issues after SSO
- [Configuration overview](/docs/model-content/configuration/overview) — the full `AdminAuthConfig` type reference
