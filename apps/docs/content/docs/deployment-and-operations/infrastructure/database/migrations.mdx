...
On startup, the SQL adapters (Postgres, SQLite, MySQL) bring the database in line with your config. They create a table for any collection that doesn't have one yet, and add a column for any newly [promoted](/docs/deployment-and-operations/infrastructure/database/indexes) field. Add a collection or a promoted field, restart, and the schema is ready.
...
`defaultValue` ensures documents always have a value when a field is omitted. When creating new documents via the SDK, API, or admin forms, Dyrected applies the `defaultValue` automatically before writing to the database:
...
Some field types also fall back to sensible empty defaults when omitted — booleans default to `false` and array fields default to an empty list.

### Backfilling defaults on existing collections

When you add a new `defaultValue` to an existing collection that already contains historical records, a subtle schema/data drift occurs:

1. **In-memory reads** surface the default value when loading documents.
2. **Database grouping and filtering** (such as operational view Kanban columns, grouped tables, or raw SQL queries) query the physical database directly, where historical rows still store `NULL`. Consequently, historical records may appear in an "Unassigned" bucket rather than their expected default status column.

To align historical records with your new default, run a one-time backfill migration:

```ts
// scripts/backfill-status-defaults.ts
import { client } from "../server/db";

async function main() {
  const { docs } = await client.collection("orders").find({
    where: { status: { equals: null } },
    limit: 1000,
  });

  for (const doc of docs) {
    await client.collection("orders").update(doc.id, {
      status: "pending",
    });
  }
  console.log(`Backfilled status default on ${docs.length} orders.`);
}

main();
```

Or execute a direct SQL query against your database adapter:
...
Adding `promoted: true` (or adding a field to an `indexes` array) creates a dedicated physical column during the next startup schema sync.

On SQL adapters (Postgres and MySQL), Dyrected automatically **backfills existing records** by extracting their values from the JSON document body into the newly promoted column. That means filtering and sorting against the freshly promoted field immediately indexes both historical and newly written rows without requiring a manual migration script. If a column was originally created as `TEXT` before an index was applied, Dyrected automatically widens it to `VARCHAR(191)` before establishing the index.
...
For the deployment side of this, see [Deployment](/docs/deployment-and-operations/production/deployment).
