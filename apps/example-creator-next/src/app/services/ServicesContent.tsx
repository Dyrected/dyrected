...
import servicesFallback from "./services-content.json";
...
export default function ServicesContent({
  ...
  servicesPageGlobal?: ServicesPageGlobal;
  services?: Service[];
}) {
  const hero = {
    badge: servicesPageGlobal?.hero?.badge ?? servicesFallback.hero.badge,
    titlePrefix: servicesPageGlobal?.hero?.titlePrefix ?? servicesFallback.hero.titlePrefix,
    titleHighlight: servicesPageGlobal?.hero?.titleHighlight ?? servicesFallback.hero.titleHighlight,
    description: servicesPageGlobal?.hero?.description ?? servicesFallback.hero.description,
  };

  const warningCallout = {
    title: servicesPageGlobal?.warningCallout?.title ?? servicesFallback.warningCallout.title,
    description: servicesPageGlobal?.warningCallout?.description ?? servicesFallback.warningCallout.description,
    actionLabel: servicesPageGlobal?.warningCallout?.actionLabel ?? servicesFallback.warningCallout.actionLabel,
  };
                ...
                >
                  ...
                  <div className="space-y-6">
                    <div className="space-y-3">
                      ...
                      <h3 className="font-heading text-xl font-bold text-white mt-2">{s.name}</h3>
      ...
      <section className="py-8 bg-[#0b0b14] border-t border-b border-white/5">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-white">{warningCallout.title}</h4>
              <p className="text-xs text-muted-foreground">{warningCallout.description}</p>
            </div>
          </div>
          <Link
            href="/assessment"
