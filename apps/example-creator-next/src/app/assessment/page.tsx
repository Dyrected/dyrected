...
export default function Assessment() {
  ...
  const handleSelectScore = (rating: number) => {
    // Save rating (1-indexed question numbers)
    const qNum = currentIdx + 1;
    setAnswers((prev) => ({ ...prev, [qNum]: rating }));

    // Automatically transition to next question with a slight delay for visual response
  ...
  const handleFinish = () => {
    ...
    // Calculate result details using category mapping
