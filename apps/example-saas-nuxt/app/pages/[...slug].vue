...
const page = computed(() => response.value?.data?.[0]);
...
useHead({
  ...
  meta: [{ name: "description", content: page.value?.seo?.metaDescription || "" }],
});
</script>
...
<template>
  ...
  <div v-else-if="slug === 'home'" class="min-h-screen flex flex-col items-center justify-center text-center px-6">
    ...
    <p class="text-slate-400 mb-8">
      ...
      <NuxtLink to="/admin" class="text-[#f5c842] underline">Admin Dashboard</NuxtLink>.
    </p>
  </div>
</template>
