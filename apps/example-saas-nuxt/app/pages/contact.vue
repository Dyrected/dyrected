    ...
    <section class="py-4 px-6 pb-20">
      <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div class="bg-card border border-border rounded-xl p-8">
          <h2 class="text-foreground font-black text-xl mb-6">Request a Snack Demo</h2>
          ...
          <div v-if="submitted" class="text-center py-10">
            ...
            <p class="text-muted-foreground text-sm">A member of our Snack Intelligence team will contact you within 1 business day to schedule your personalized demo and complimentary snack audit.</p>
          </div>
          ...
          <form v-else class="space-y-5" @submit.prevent="handleSubmit">
            ...
            <div>
              ...
              >
                <option value="" disabled class="text-muted-foreground">Select drawer count</option>
            ...
            <div>
              <label class="block text-muted-foreground text-sm font-medium mb-1.5" for="message">Tell us about your snack situation</label>
              <textarea
                id="message"
                ...
                class="w-full bg-muted border rounded-lg px-4 py-3 text-foreground text-sm placeholder-muted-foreground focus:outline-none focus:border-intelligence/50 transition-colors resize-none"
            ...
            <button
              type="submit"
              class="w-full py-3.5 bg-primary text-primary-foreground font-black rounded hover:bg-primary/90 transition-colors text-sm disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              :disabled="submitting"
            >
            ...
            <p class="text-muted-foreground text-xs text-center">By submitting, you agree to receive snack-related communications. You can unsubscribe at any time by emailing unsubscribe@snacktrack.pro.</p>
          </form>
        </div>
        ...
        <div class="space-y-4">
          <div
            ...
            :key="contact.label"
            class="bg-card border border-border rounded-xl p-6 hover:border-intelligence/20 transition-colors"
          >
            <div class="flex items-start gap-4">
              <div class="w-10 h-10 bg-intelligence/10 border border-intelligence/20 rounded-lg flex items-center justify-center shrink-0">
                <svg class="w-5 h-5 text-intelligence" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path v-html="contact.icon" />
                </svg>
              </div>
              <div>
                <h3 class="text-foreground font-bold text-sm mb-1">{{ contact.label }}</h3>
                <a :href="`mailto:${contact.email}`" class="text-intelligence text-sm hover:underline">{{ contact.email }}</a>
                <p class="text-muted-foreground text-xs mt-2 leading-relaxed">{{ contact.desc }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-card border border-border rounded-xl p-6">
            <h3 class="text-foreground font-bold text-sm mb-5">Global Offices</h3>
            <div class="grid grid-cols-2 gap-4">
              <div v-for="office in offices" :key="office.city" class="space-y-1">
                <p class="text-intelligence text-xs font-bold uppercase tracking-wide">{{ office.city }}</p>
                <p class="text-muted-foreground text-xs leading-relaxed whitespace-pre-line">{{ office.address }}</p>
              </div>
            </div>
          </div>
        </div>
