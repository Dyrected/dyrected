...
<template>
  <section class="py-16 px-6 bg-navy-900 border-t border-navy-700">
    <div class="max-w-3xl mx-auto">
      <div v-if="heading" class="text-center mb-12">
        <SectionLabel text="Common Questions" />
        <h2 class="text-3xl font-black text-white mt-2">{{ heading }}</h2>
      </div>
      <div class="space-y-3">
        <div
          v-for="(faq, i) in items"
          :key="i"
          class="border border-navy-700 rounded-xl overflow-hidden"
        >
          <button
            class="w-full flex items-center justify-between px-6 py-5 text-left hover:bg-navy-800/50 transition-colors"
            @click="toggle(i)"
          >
            <span class="text-white font-semibold text-sm pr-4">{{ faq.question }}</span>
            <svg
              class="w-5 h-5 text-gold-400 shrink-0 transition-transform"
