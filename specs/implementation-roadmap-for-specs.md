...
| 0️⃣ | [database-migrations-and-schema-evolution-spec.md](file:///Users/busola/Work/dyrected/specs/database-migrations-and-schema-evolution-spec.md) | **Approved for Implementation** | Dual Engine Model (`sync()` dev velocity + `dyrected migrate` production release gate), `defineMigration({ up, down })` API, `dyrected_migrations` ledger across MySQL/Postgres/SQLite/Mongo, advisory locks, and CLI commands (`migrate`, `migrate:create`, `migrate:status`, `migrate:rollback`). |
