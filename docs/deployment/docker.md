...
|---|---|---|
| `/app/public/uploads` | Files from `LocalStorage` adapter | Not needed if using S3/Cloudinary |
...
> If you use `LocalStorage` for file uploads, the `uploads` volume must be mounted. For production, prefer an S3-compatible storage adapter — it works across multiple replicas and doesn't need a volume.
