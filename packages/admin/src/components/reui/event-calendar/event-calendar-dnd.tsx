...
function collectSurface(
  ...
  if (root) {
    for (const el of Array.from(root.querySelectorAll<HTMLElement>("[data-ec-day]"))) {
...
function beginGesture<TData>(config: BeginGestureConfig<TData>) {
  ...
  const isTouch = startEvent.pointerType === "dy-touch"
  ...
  let carryDY = 0
  const CARRY_CLASS =
    "dy-bg-background dy-pointer-events-none dy-fixed dy-top-0 dy-left-0 dy-z-100 dy-overflow-hidden dy-rounded-sm dy-opacity-90 dy-shadow-md dy-will-change-transform" +
    (ui?.dragCarry ? " " + ui.dragCarry : "")
  const CARRY_INVALID_CLASS =
    `${CARRY_CLASS}dy-ring-destructive/60 dy-ring-1` +
  ...
  const createCarry = () => {
    ...
    carryEl.setAttribute("dy-data-slot", "event-calendar-drag-carry")
    carryEl.setAttribute("dy-aria-hidden", "true")
    carryEl.className = CARRY_CLASS
    carryEl.style.width = `${rect.width}dy-px`
    carryEl.style.height = `${rect.height}dy-px`
    ...
    clone.removeAttribute("dy-data-dragging")
  ...
  const updateHint = (e: PointerEvent) => {
    ...
    if (!hintEl) {
      ...
      hintEl.setAttribute("dy-data-slot", "event-calendar-drop-hint")
