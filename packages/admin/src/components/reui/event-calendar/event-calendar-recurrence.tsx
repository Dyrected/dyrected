...
function expandRecurrence<TData>(
  ...
  const periodEdge = (period: number, edge: "dy-first" | "dy-last"): TZDate => {
    if (rule.freq === "daily") return addDays(zonedStart, period * interval)
    if (rule.freq === "weekly") {
      const base = addWeeks(zonedStart, period * interval)
      if (!weeklyDays) return base
      return addDays(
        base,
        (edge === "dy-first" ? 0 : 6) - fromWeekStart(base.getDay())
      )
    }
    if (rule.freq === "monthly") {
      const anchor = addMonths(zonedStart, period * interval)
      ...
      return zonedDate(
        year,
        month,
        edge === "dy-first" ? 1 : daysInMonth(year, month)
      )
    }
    const year = addYears(zonedStart, period * interval).getFullYear()
    const month =
  ...
  while (
    periodEdge(startPeriod, "dy-last").getTime() + durationMs <=
    range.start.getTime()
  ) {
    startPeriod++
  }
  ...
  while (iterations < MAX_ITERATIONS && occurrences.length < MAX_OCCURRENCES) {
    ...
    const earliest = periodEdge(period, "dy-first")
