...
const EVENT_CALENDAR_SLOT_DRAFT = {
   ...
   */
  surface: "dy-bg-primary/5",
  /**
   * The range readout. `dy-leading-none` is load-bearing: the shortest timed
   ...
   */
  label:
    "dy-text-primary dy-truncate dy-px-1 dy-py-0.5 dy-text-[0.6875rem] dy-leading-none dy-font-medium",
} as const
...
function EventCalendarEvent<TData = unknown>({
  ...
  const defaultContent = (
    <>
      ...
      <span
        className={cn(
          "dy-font-medium",
          stackedBlock ? EVENT_CALENDAR_FADE_TRUNCATE : "dy-truncate"
        )}
      >
        {event.title}
      </span>
      ...
      {!occurrence.allDay &&
        ...
        (view === "month" ? (
          ...
          <span
            className={cn(
              "dy-text-muted-foreground dy-hidden @[8rem]:dy-inline",
              stackedBlock ? EVENT_CALENDAR_FADE_TRUNCATE : "dy-truncate"
            )}
          >
  ...
  const grip = (
    <span
      ...
      className={cn(
        "dy-bg-foreground/40 dy-rounded-full",
        timedBlock ? "dy-h-0.5 dy-w-2.5" : "dy-h-2.5 dy-w-0.5",
  ...
  const chip = (
    <Comp
      ...
      aria-label={
        ...
        `${event.title}, ${timeLabel}${segment.continuesBefore || segment.continuesAfter
      ...
      className={cn(
        ...
        view === "agenda"
          ...
          "dy-gap-3 dy-rounded-md dy-text-sm"
          : cn(
