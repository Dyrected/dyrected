...
function useActionRunner(options: {
  ...
  const keyOf = useCallback((actionName: string, ids: string[]) => `${actionName}::${[...ids].sort().join(",")}`, []);
  ...
  const initiate = useCallback(
    (action: ActionLike, ids: string[], targetContext?: ActionTargetContext) => {
      const isHeader = action.type === "header";
      if ((!ids.length && !isHeader) || !action.name) return;
  ...
  return useActionRunner({
    runRequest: (action, input) => {
      ...
      return (client as any).collection(slug).runAction(viewSlug, action.actionName, {
        ...(action.ids.length === 1 ? { id: action.ids[0] } : action.ids.length > 1 ? { ids: action.ids } : {}),
        input,
      });
    },
    onSuccess: async () => {
