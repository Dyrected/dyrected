...
import { buildDocsRuntimeManifest } from "./docs-runtime.mjs";
...
const generatedRuntimeSource = path.join(packageRoot, "src/generated/runtime.ts");
...
const {
  manifest: docsRuntimeManifest,
  warnings: docsRuntimeWarnings,
  failures: docsRuntimeFailures,
} = buildDocsRuntimeManifest({
  allDocsRoot,
  repositoryRoot,
  filenames: walkFiles(allDocsRoot, ".mdx"),
});
for (const warning of docsRuntimeWarnings) console.warn(`[knowledge] ${warning}`);
if (docsRuntimeFailures.length) {
  throw new Error(
    `Docs runtime metadata validation failed:\n- ${docsRuntimeFailures.join("\n- ")}`,
  );
}
outputFile(
  path.join(generatedRoot, "docs-runtime-manifest.json"),
  `${JSON.stringify(docsRuntimeManifest, null, 2)}\n`,
);
outputFile(
  generatedRuntimeSource,
  `/* Generated by scripts/generate.mjs. Do not edit manually. */\nimport type { DocsRuntimeManifestEntry } from "../types.js";\n\nexport const docsRuntimeManifest: readonly DocsRuntimeManifestEntry[] = ${JSON.stringify(docsRuntimeManifest, null, 2)};\n`,
);
function filterRuntimeOnlyBlocks(source, siteRuntime) {
  const keepTag = siteRuntime === "cloud" ? "CloudOnly" : "SelfHostedOnly";
  const dropTag = siteRuntime === "cloud" ? "SelfHostedOnly" : "CloudOnly";
  const dropBlock = new RegExp(
    `\\n?<${dropTag}>\\s*\\n[\\s\\S]*?\\n\\s*</${dropTag}>\\n?`,
    "g",
  );
  const keepOpen = new RegExp(`\\s*<${keepTag}>\\s*\\n?`, "g");
  const keepClose = new RegExp(`\\n?\\s*</${keepTag}>\\s*`, "g");
...
const normalizeMdx = (source, siteRuntime) =>
  filterRuntimeOnlyBlocks(source, siteRuntime)
    .replace(/^---[\s\S]*?---\s*/, "")
    .replace(/^import .*$/gm, "")
    .replace(/\{\/\*[\s\S]*?\*\/\}/g, "")
    .trim();
const docsByRelativePath = new Map(
  docEntries.map((entry) => [entry.relative, entry]),
);

function toRuntimeAbsoluteDocsUrl(siteRuntime, docsPath) {
  return `https://docs.dyrected.com${docsPath.replace(
    /^\/docs\//,
    `/docs/${siteRuntime}/`,
  )}`;
}

function siteRuntimeSupports(entryRuntime, siteRuntime) {
  return (
    entryRuntime === "shared" ||
    entryRuntime === "variant" ||
    entryRuntime === siteRuntime
  );
}

function getRuntimeUrl(entry, siteRuntime) {
  return siteRuntime === "cloud" ? entry.urlCloud : entry.urlSelfHosted;
}

const runtimeDocEntries = docsRuntimeManifest.map((entry) => {
  const authored = docsByRelativePath.get(entry.relativePath);
  if (!authored) {
    throw new Error(
      `Missing authored source for runtime docs entry: ${entry.relativePath}`,
    );
  }
...
function renderRuntimeFullDocs(siteRuntime) {
  return runtimeDocEntries
    .filter((entry) => siteRuntimeSupports(entry.runtime, siteRuntime))
    .map((entry) => {
      const url = getRuntimeUrl(entry, siteRuntime);
...
const docsRuntimePreamble = [
  "Dyrected is a code-first content platform for custom websites.",
  "",
  "Dyrected Cloud is a managed content backend for website content, media, content APIs, editor access, publishing workflows, and Cloud-safe hooks defined as content rules. Cloud content events and webhooks are coming soon.",
  "",
  ...
  "",
  "This runtime framing is derived from the July 29, 2026 product-boundary and docs-architecture specs and should be human-reviewed before being treated as final product positioning.",
].join("\n");
const selfHostedIntentLines = recipes.flatMap((recipe) =>
  recipe.intents.map(
    (intent) =>
      `- “${intent}” → [${recipe.title}](${toRuntimeAbsoluteDocsUrl("self-hosted", recipe.docsPath)})`,
  ),
);

const llmsIndexContent = `# Dyrected\n\n${docsRuntimePreamble}\n\n## Runtime-specific indexes\n\n- [Dyrected Cloud concise index](https://docs.dyrected.com/llms-cloud.txt)\n- [Self-hosted Dyrected concise index](https://docs.dyrected.com/llms-self-hosted.txt)\n- [Dyrected Cloud full docs](https://docs.dyrected.com/llms-full-cloud.txt)\n- [Self-hosted Dyrected full docs](https://docs.dyrected.com/llms-full-self-hosted.txt)\n- [Combined compatibility docs](https://docs.dyrected.com/llms-full.txt)\n\n## Intent-to-pattern index\n\n${selfHostedIntentLines.join("\n")}\n`;

outputFile(docsPublicRoot + "/llms.txt", llmsIndexContent);
outputFile(docsPublicRoot + "/llm.txt", llmsIndexContent);
outputFile(
  docsPublicRoot + "/llms-cloud.txt",
  `# Dyrected Cloud\n\n${docsRuntimePreamble}\n\n## Documentation\n\n${renderRuntimeConciseDocs("cloud")}\n`,
);
outputFile(
  docsPublicRoot + "/llms-self-hosted.txt",
  `# Self-hosted Dyrected\n\n${docsRuntimePreamble}\n\n## Documentation\n\n${renderRuntimeConciseDocs("self-hosted")}\n`,
);
outputFile(
  docsPublicRoot + "/llms-full-cloud.txt",
  `# Dyrected Cloud complete documentation\n\n${docsRuntimePreamble}\n\n${renderRuntimeFullDocs("cloud")}\n`,
);
outputFile(
  docsPublicRoot + "/llms-full-self-hosted.txt",
  `# Self-hosted Dyrected complete documentation\n\n${docsRuntimePreamble}\n\n${renderRuntimeFullDocs("self-hosted")}\n`,
);
outputFile(
